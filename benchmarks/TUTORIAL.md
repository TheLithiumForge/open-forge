...
- A **primitive** is one directive, memory, pattern, workflow, skill, workspace route, local tool, or other worker-visible input.
...
| Arm | Installed guidance | Expected behavior |
| --- | --- | --- |
...
RUNS_ROOT = "D:/OpenForgeBenchmarkRuns/ledger-example"
```
...
bun run bench -- prepare ledger-immutable-control `
  --runs-root D:\OpenForgeBenchmarkRuns\ledger-example
...
```powershell
bun run bench -- finish D:\OpenForgeBenchmarkRuns\ledger-example\<control-run-id>
bun run bench -- finish D:\OpenForgeBenchmarkRuns\ledger-example\<trap-run-id>
```
...
- naturally found the immutable-removal guidance;
...
The CLI remains optional. A manual run may reproduce the same plain-file contract as long as it preserves frozen inputs, worker-orchestrator separation, an honest trace boundary, independent checks, and both reviews.
