...
Keep benchmark and framework sources read-only. Write only inside the external
run-set directory, its prepared arm directories, and disposable check copies.
Do not edit stable scenarios, primitives, meta-scenarios, prompts, review
criteria, framework sources, or historical results during a run set.
...
4. Prepare and validate every arm. Do not launch a worker while another arm's
...
a treatment after any worker outcome is visible.
```
