...
> Work in the directory `<target>`. There's existing work in progress from a previous session (I have no memory of it — you're on your own to figure out the current state). <one-line task>. There is an AGENTS.md file at the repo root — take a look if it seems relevant. When done, report: what state you found, what helped you understand it quickly, what you changed, what you verified. Do not ask me clarifying questions.
