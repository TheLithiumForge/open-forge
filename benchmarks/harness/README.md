...
The [workflow-first scenario set](scenarios/workflow-first/README.md) adds five explicitly **UNRUN** external-worker definitions for exact workflow selection, no-match choice, explicit opt-out, ordered handoffs, and provenance/accord-aware vision and greenfield architecture support derivation. Automated tests validate their run-specs and prepare their worker-visible compositions through the real CLI in OS temporary directories. That is plumbing evidence only; behavioral conclusions require fresh external model runs with interaction and tool traces under the frozen rubric.
