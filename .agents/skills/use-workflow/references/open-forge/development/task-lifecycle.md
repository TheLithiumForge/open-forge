---
open-forge:
  description: Prepare and run one task from an exact local baseline with a compact execution capsule, protected surfaces, and coherent checkpoints
...
Prepare and run one implementation task from an exact local baseline with accepted architecture, bounded ownership, resumable context, and truthful checkpoints, without assuming a branch name, commit cadence, or remote action.
...
2. Use the [Work Records Pattern](../../../../../patterns/work-records.md) for record relationships and keep the profile-specific fields below. Create or refresh one compact execution capsule containing accepted decisions, invariants, placement map, expected paths, protected paths, direct integration neighborhood, dependencies, evidence ladder, consumed-budget IDs, stop conditions, current owner, completed boundary, and next action.
...
When a durable task record is useful, keep these sections concise:

- `Outcome And Profile`
- `Authority And Accepted Architecture`
- `Execution Capsule`
...
Link detailed sources and logs instead of copying them.

## Completion

- The task used an exact local baseline and preserved unrelated work.
