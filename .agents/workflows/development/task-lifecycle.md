...
5. The Mastermind performs integrated inspection of changed paths, the diff, and reproduced evidence after phase results and before accepting a commit. The Mastermind owns exact-path formatting and derived-state regeneration when Preflight or repository rules require them.
6. Preserve coherent, readable feature commits. Only the Mastermind stages and commits accepted work after inspecting the actual result. Helpers do not edit Task state, stage, commit, merge, push, or accept work.
7. Keep one concise authoritative Task record under the applicable Working Memory route. Update it with current progress, decisions, commits, evidence, correction disposition, residual risk, and completion rather than creating competing status records. When a phase changes owned files, record its progress in the same coherent commit; a no-change phase carries its evidence forward without a status-only commit.
...
Keep one concise authoritative Task record that links to accepted sources and contains these sections:
...
- `Progress` records Preflight, Gray, Red, Green, Blue, Purple, public scenario, full gate, optional independent review, correction, and Acceptance with compact evidence links.
...
- Only the Mastermind committed accepted work, and the feature commits remain coherent and readable.
