...
3. Before Green, require complete affected failing evidence for every accepted behavior row and material behavior class. Safety, destructive, compatibility, regression, boundary, failure, and externally visible behavior must be explicit before production mutation. Representative-only Red evidence belongs to a Standard profile and must not be used to shorten this Assured phase.
...
- Every accepted affected behavior row and material behavior class has frozen expectation meaning and failing evidence.
