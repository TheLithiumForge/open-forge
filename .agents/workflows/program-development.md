...
3. **Build the golden slice.** Apply the appropriate standard or assured workflow to the first item of each archetype. Capture the accepted architecture, placement map, contract pattern, implementation shape, test harness, public scenario, and known failure modes. Run an early architecture checkpoint after the first representative connected slice crosses a shared, public, or module-responsibility boundary.
...
6. **Run parallel lanes.** Use parallel execution only within existing authority and available isolation. Otherwise execute the same dependency order sequentially. Execute non-overlapping derivative slices concurrently when their contracts and shared inputs are stable. Give each slice one owner and one integration point. Do not parallelize owners over the same mutable foundation.
7. **Review and gate by batch.** Run focused evidence per slice. Review a representative or triggered target rather than every repetition, then run integration, public, and full gates at a coherent batch boundary. When coordinated review is selected, one read-only Review Mastermind may validate a bounded list of independently owned snapshot records over exact commits from several worktrees in the same repository. It preserves each snapshot's semantic owner, original return writer, actual lineage, receipts, and budget units, keeps at most one wave per task snapshot, and joins returns by snapshot and writer. Escalate any slice that introduces a new public contract, shared foundation, safety boundary, or materially different behavior back to foundation treatment.
8. **Learn from yield.** Treat observations as evidence for a proposal, not authority to bypass required checks. Replacing a required review or gate needs acceptance within its scope. Track accepted material review findings, rework, context reloads, repeated tool failures, corrections from people, and critical-path time. Remove a recurring review with near-zero yield, encode repeated findings as tests or rules, and lower rigor only after comparable slices remain correct.
