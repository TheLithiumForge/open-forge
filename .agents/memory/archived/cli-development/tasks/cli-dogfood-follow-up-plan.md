...
open-forge:
  ...
  tags: [Memory, Archived, Contextual, Historical, CLI, Task, Plan, Testing, Parsing, Doctor, Dogfood]
---
...
## Terminal Initialization Observation

The colour trial found an existing runtime keypad-initialization prefix in raw
terminal stdout, including JSON/version with NO_COLOR. Set 6 and Set 7 reproduce
it; piped JSON remains clean. The [presentation capsule](cli-presentation-implementation.md#observed-host-terminal-initialization-separate-follow-up)
records exact reproduction, runtime evidence and the separate host investigation
boundary. No workaround or changed cancellation behavior is part of colour.
