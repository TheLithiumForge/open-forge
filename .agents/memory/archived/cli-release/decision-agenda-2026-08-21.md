---
open-forge:
  description: Historical accepted direction and decision agenda from the removed first C# CLI implementation program
  tags: [Memory, Archived, Contextual, Historical, CLI, Release, Gate, Decision, Discussion]
---

# CLI Decision Agenda

This is the sealed decision agenda from the implementation program reset on
2026-08-21. It preserves product and architecture rationale but does not define
the new greenfield implementation. Validate any reused decision against the
current CLI Architecture and contracts.
...
- [x] **CLI-D093 — Full delivery order:** after implementation readiness, deliver
      ...
      The [Release Plan](release-plan.md#durable-implementation-sequence) records
      the command-by-command dependency order. A Task may move only when
      read-only Preflight records concrete dependency evidence. Create thin
      package-manager packaging with npm as the first wrapper only after every
      retained command, then finish with CI for build, test, and release.
      Wrappers never implement command behavior, and release or publish actions
      run only from `main`. Gate 5 has no partial publication target.
...
- [x] **CLI-D096 — One executable and physical solution:** use one production
      ...
      `src/open-forge-cli/OpenForge.Cli`, with exactly the independently runnable
      `OpenForge.Cli.UnitTests`, `OpenForge.Cli.IntegrationTests`, and
      `OpenForge.Cli.EndToEndTests` projects under `tests/open-forge-cli` plus one
      source-only `OpenForge.Cli.TestSupport` folder. Test projects are not shipped
      executables.
...
- [x] **CLI-D104 — Modern managed source and artifact layout:** keep
      `OpenForge.slnx` as direct project membership with no presentation-only
      solution folders. SDK default compile globs own authored C# inclusion;
      explicit compile items are limited to deterministic generated source and
      deliberately out-of-tree source-only test support. Supporting replacement
      CLI implementation lives below an explicit `Shared/<Capability>/` child of
      its narrowest owning CLI, command-family, or command-leaf boundary, with
      matching namespaces and mirrored focused tests. A private `Shared` folder
      states ownership, while promotion to a wider `Shared` parent still requires
      a real identical consumer. Centralize .NET build artifacts through
      `Directory.Build.props` at `dist/dotnet/`; project-local `bin/` and `obj/`
      output is not accepted. Keep one managed production project that publishes
      all six RIDs, and author no custom native source, P/Invoke, manual OS
      binding, or per-platform production project. Use the BCL first, consider
      only a focused managed Native-AOT-safe dependency for a proved critical
      gap, simplify a minor product detail when its platform cost is
      disproportionate, and return
      to Architecture before native or interop work.
...
- [ ] **CLI-D105 — Managed scoped Framework route instances:** before the `route init` implementation is finalized, decide whether to extend its accepted
      generic chain initialization with `route init <concrete-route> --framework`.
      The candidate uniquely aligns non-root Framework route segments from the
      embedded canonical route topology while treating inserted concrete segments
      as user-owned scopes, creates only the requested sparse chain, projects
      destination-local generated `Entries`, and records dynamic scoped Framework
      lifecycle identity for later update. Ambiguous alignment blocks, root routes
      cannot be recreated or scoped, and static Framework install remains a closed
      footprint. This candidate also requires explicit later remove/release and
      lifecycle decisions. It does not block `route list`, `route inspect`, or
      other earlier read-only Tasks and must not be improvised by implementation.
