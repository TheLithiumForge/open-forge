...
| `responsible person` or `responsible role` | Human accountability for work or a decision.                                                              | Do not confuse responsibility with semantic authority.                             |
...
| `self-growing`  | Able to expand through useful records and routed scopes as work produces knowledge.     | Growth is deliberate and has no fixed structural ceiling. It does not mean automatic recording.                 |
...
| `Handoff`       | Sealed snapshot of an actual transfer or explicitly planned resumption boundary.        | Create it when boundary state must outlive Checkpoint changes. Never edit it after sealing.                     |
...
| `prune`          | Remove Working or candidate material that is no longer useful.                 | Do not prune useful history.                                                     |
...
| `description`    | Short text that helps a reader choose or skip a route before opening it.       | State the purpose, trigger, or useful result.                                    |
