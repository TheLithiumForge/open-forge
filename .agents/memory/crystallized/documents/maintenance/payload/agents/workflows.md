...
open-forge:
  ...
  responsibility: Preserve Workflow selection, recipe validation, dependency, composition, local Core, source alignment, and route boundaries
...
The [current Workflow contract](../../../framework/primitives/workflows.md) defines recipe structure, selection, dependencies, composition, and workflow-local Core.
...
- Workflow-local Core routes retain their explicit primitive type and reuse the ordinary Framework instead of creating a nested installation
- The installable source begins with no opinionated Workflow recipes
## Verification

- Workflow validation tests cover section level and order, Mode, Constraints, phase classification, Required Routes, organizational entrypoints, and local primitive ownership
