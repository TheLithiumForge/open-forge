...
The `description` of an `entry` is the pre-load selection surface. It communicates enough trigger, purpose, or outcome to choose the `route` without opening its body.
...
The loader's [universal routing rules](../../../../../loader.md#routing) apply to every `route` tree. The [scope and inheritance contract](scope.md) explains their placement, management, and authority consequences.

Recursive depth does not change navigation. Each selected `entrypoint` still exposes only direct children, and selection continues one routed boundary at a time.

Management creates no parallel navigation surface. Managed and unmanaged `routes` are selected through the same `entrypoint` and `entry` contract.
