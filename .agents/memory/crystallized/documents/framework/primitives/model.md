...
Core currently defines six primitives: Directives, Guidance, Patterns, Skills, Templates, and Map. Workflow recipes are expressed through Skills rather than a separate Core primitive. The loader, routing, loading, and inherited `Axioms` are Core mechanics that make these roles work, not additional primitives. Direct Directive leaves use `Instructions`; `Axioms` remain the inherited mechanics of the loader and entrypoints.
...
Workflow recipes remain a useful procedure term. The native [Use Workflow Skill](../../../../../skills/use-workflow/SKILL.md) selects and follows them; a recipe is not a seventh primitive or a separate `root route`.
...
Guidance remains advisory. A Pattern is the established reusable shape in its selected scope, but a different shape may be chosen deliberately; a mandatory shape also needs a Directive or another binding `axiom`. A Skill follows its native `SKILL.md` contract and active runtime. A Map `route` points to detailed truth without replacing it. A Workflow recipe is selected through a Skill when its goal fits; selection does not grant the recipe independent authority or replace the Skill's execution rules.

## Composition

Primitive relationships are compositional rather than one authority ladder.

A selected Workflow recipe may invoke Skills, consult Guidance, apply Patterns, instantiate Templates, and follow Map `routes` while obeying every Directive active in its scope. Any primitive may link to another when the relationship helps selection or execution, but the link does not merge their semantics.
...
Core `root routes` compose through explicit relationships. A Skill may select and follow a Workflow recipe, which can invoke Skills, consult Guidance, apply Patterns, instantiate Templates, and follow Map `routes` through Steps or ordinary links instead of physically containing those `root routes`.
...
Recorded text does not become active behavior merely because it describes a Directive, Pattern, Workflow recipe, or another primitive. When durable behavior is intended, the accepted material is promoted or linked to the matching Core `route`.
...
A repeatable Workflow recipe does not require expanding the shared primitive vocabulary. The native Skill mechanism supplies its selection and execution boundary.
...
- [Use Workflow Skill](../../../../../skills/use-workflow/SKILL.md)
