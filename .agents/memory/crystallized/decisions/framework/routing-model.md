...
Open Forge uses small recursively linked Markdown `entrypoints` with direct-child `entries`. Selection scans visible surfaces, recursively selects materially relevant scopes, composes selected chains, and reevaluates after a material task change without loading route bodies merely to expose them.
