...
- Unconditional applicability evaluation outperforms guessing from tags or placement. Read every direct directive far enough to evaluate its explicit `Applies To`, then load and obey the full file when that positive scope matches. The earlier workspace-wide directive result remains evidence for early evaluation, not permission to infer scope from root placement.
