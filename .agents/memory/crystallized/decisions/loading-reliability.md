...
Route selection establishes scope before routed content is read. Directives use ordinary #LoadNow traversal after their route establishes scope, without a second applicability gate inside each directive. Continuity uses the workspace-wide routed #KeepInMind set and each result's visible #LoadNow closure rather than only the active parent chain.
...
- Restricting #KeepInMind to closeout or the active parent chain would create continuity gaps
...
- Baseline directives and continuity checkpoints must remain compact
...
- `open-forge load --bodies` remains an optional deterministic accelerator over the complete manual traversal
