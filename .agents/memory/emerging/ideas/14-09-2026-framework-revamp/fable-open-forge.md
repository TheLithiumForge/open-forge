# Open Forge, rewritten (first pass)

This is a like-for-like rewrite of every file in `src/open-forge/`, done before reading `.agents/directives/public-facing-writing.md`. Loading tags, frontmatter, headings, and the set of rules are unchanged unless a note says otherwise. The goal was to keep every rule while making each one faster to read and harder to misread.

What changed, and why:

- **The agent is addressed as "you".** The files are instructions to an agent. Second person is shorter, and it removes the ambiguity of "the agent" versus "the user" in sentences that mention both.
- **Terms are defined once, up front.** `route`, `scope`, and `accepted` were used before they were explained. `slug` is gone from the running text; "scope folder" says the same thing without a new word.
- **The loader's sixteen authority axioms are grouped by the question they answer.** What wins. Working with the user. Where meaning lives. How rules combine. Same rules, in an order a reader can hold.
- **One general "look at the Entries before deciding nothing applies" rule now lives in the loader.** Each category keeps a one-line trigger ("Look here when ...") instead of restating the rule in slightly different words.
- **The three "manager" axioms collapse to one sentence.** How a manager recognizes route shapes is a CLI contract, not something an agent needs at every startup. The one fact an agent does need, that scoping must not reorder managed segments, is stated where scoping is explained.
- **The Checkpoint and Handoff entrypoints no longer both explain when to create a Handoff.** Handoffs owns that rule. Checkpoints points to it.
- **The "Analysis terminates in a Task" axiom is reworded in base vocabulary.** "Task" and "subtasks" are Planning Extension words. The rule itself is sound and stays.
- **The restatement sentence under each category question is dropped or made to carry new information.** "## What behavior is required in this scope?" followed by "Directives state the instructions that must be followed within their scope." said the same thing twice.
...
This workspace runs on Open Forge. Its rules and context live in `.agents/`.

Before you start a task, read `.agents/loader.md`. Follow the routes that matter for the task, including nested scopes, and keep the loaded rules in force until the task ends.
<!-- open-forge:end -->
```
...
Read this after `AGENTS.md`. It tells you how the workspace is organized, what wins when sources disagree, how to work with the user, and where new knowledge belongs.
...
- `route` - A path you can follow through entrypoints and their entries.
- `root route` - A route this loader exposes directly.
- `scope` - A folder inserted below a root that narrows everything beneath it.
- `description` - The one-line summary that helps you decide whether to open a file.
...
- A request to act lets you make the routine, reversible, in-scope choices the work needs.
...
- Match explanation and planning to the request. Unless the user asks for deeper analysis, open with your current understanding, one recommendation, and at most one important open choice.
- Keep going through safe, in-scope work when accepted direction or a stated reversible assumption covers it. Do not stop only to ask whether to continue.
...
- Narrower non-binding material may specialize broader material of the same kind. Binding instructions add up; they never silently override one another. Report conflicts you cannot resolve.
...
- Follow relevant branches recursively. Every folder on a route has exactly one recognized entrypoint. Do not open file bodies just to discover routes.
- Before deciding a category has nothing for the task, look at its `Entries`.
...
- Keep separately selected scopes as separate chains and follow their explicit links. Recheck your selection after an important change in the task.
...
- #Evergreen - Must stay aligned with accepted current state. Update it before dependent work relies on it and no later than closeout; batch related updates when safe. Preserve useful prior context in the right Memory route, and report updates you cannot make.
...
- Every Directive listed under an entrypoint's `Entries` carries #LoadNow.
- Each Directive has one non-empty `## Instructions` section.
- Directives listed by this root entrypoint apply across the workspace.
- A child entrypoint narrows the scope first. The Directives it lists apply only within that route.
- Child Directives add to the parent Directives already in force. A narrower scope grants no higher authority.
- If an instruction conflicts with another or cannot be followed, say so and explain why.
...
Guidance is advice, not a rule. Adapt it to the situation in front of you.

## Axioms

- Look here when the work involves a recurring situation or choice.
...
Use this when you are exploring an idea, resolving an important uncertainty, clarifying what the user wants, or finishing a broad piece of work.
...
- Unless the user asks for deep analysis, open with the outcome as you understand it, your strongest recommendation, and at most one important open choice.
...
- Make it obvious which points are accepted direction, which are recommendations, which are assumptions, and which are open questions.
...
- Ask only when the answer could materially change the outcome, boundary, risk, reversibility, or your authority to proceed.
...
When context isolation is available, start the reviewer without the implementation discussion. Give it the accepted goal, the rules, the workspace, and the resulting changes, and require it to find the relevant sources itself. Without isolation, call the check an adversarial second pass, not an independent review. Keep either one read-only unless changes are separately authorized.
...
- Look here when the work needs an important local or external source.
- Each Map links to one or more destinations and says what they contain and when they matter. The destination keeps its own detail and authority.
...
- Look here when the work creates, changes, or reviews something with a visible structure.
- A Pattern defines one concrete shape: code, files, naming, placement, boundaries, an API, a document, or another inspectable result.
- Create or update a Pattern only when an accepted shape should guide future related work. One-off work, a temporary transition, or an unsettled candidate is not a Pattern just because it has structure.
...
- Treat an applicable Pattern as the default shape in its scope. Depart from it only for a deliberate reason, and explain the departure before work depends on it. If departing from an agreed shape needs authority you do not have, ask first.
...
Skills are native `SKILL.md` packages. Each one follows its own `SKILL.md`.

## Axioms

- Look here when a Skill might help.
- Follow the selected `SKILL.md` for its metadata, use, instructions, and resource loading.
- Your runtime controls how Skills are activated, invoked, installed, and run.
...
Copy a Template, adapt it, then own the result. The Template does not follow it.

## Axioms

- Look here when copy-ready starting content would help with a new artifact.
- Pick the most relevant Template. Copy and adapt only what the destination needs.
...
- Say in each Template's description and instructions what need and primary question it answers.
...
Workflows are optional recipes. Work directly when none adds value.

## Axioms

- Look here when the user names a Workflow, or when a description shows the recipe would clearly help the goal.
...
- Prefer the smallest Workflow that resolves an important missing decision or execution risk. Installed Workflows are never mandatory stages, and users never have to name them.
- When a Workflow noticeably changes the interaction, say in one natural sentence which approach you are using and why. Route details are optional.
...
- Every Workflow file listed here is a complete recipe: one non-empty section for each of `## Goal`, `## Steps`, and `## Completion`, in that order. An entrypoint may omit them only when it just organizes descendants. Any file that uses one of these sections must have all three.
...
Memory grows with the work. It holds Markdown records for active work, coordination, accepted knowledge, candidates, and history. People and agents add records and routed scopes deliberately. There is no fixed structural limit, and unselected branches stay out of the active context.
...
- Look here before deciding no working memory applies.
- Working Memory helps work resume. Saving something here does not make it accepted. It may hold an explicitly accepted temporary choice when its source, scope, and expected expiration are clear.
...
A Checkpoint is the live state of one active workstream. Update it as the work moves. It is neither history nor accepted truth.
...
- Update it before an actual transfer or explicitly planned resumption. If that boundary needs a snapshot that will not change, create a Handoff. Routine pauses and ordinary closeout do not.
...
A Handoff is a sealed snapshot for another reader. It preserves what is needed to resume after an actual transfer or an explicitly planned resumption across a context boundary.

## Axioms

- Look here when work is resumed, transferred, delegated, interrupted, or reviewed after a context break.
- Handoffs are transfer notes, not complete history or accepted truth.
- Create and seal one when an actual transfer or explicitly planned resumption needs a stable snapshot while the Checkpoint keeps changing. Do not create one for a routine pause, ordinary closeout, or a possible future interruption.
...
Emerging Memory holds findings, possibilities, and reasoning that are useful but not yet accepted.
...
- Look here when current work needs prior reasoning.
...
- Look here when the work explores possibilities, plans future work, revisits postponed options, or needs earlier exploration.
...
An Observation is something concrete you noticed in evidence that may matter after the current work: a surprising result, a recurring failure, a gotcha the next session would otherwise rediscover.

## Axioms

- Before handoff or closeout, record an Observation when a concrete occurrence or pattern may matter later. If you cannot write it, report that.
...
- Look here before deciding no crystallized memory applies.
...
- Look here when the work needs the reason behind an important choice.
...
A Document explains accepted current knowledge as one coherent view.

## Axioms

- Look here when the work needs accepted current knowledge.
- Explain current meaning so a reader can use it without piecing it together from Decisions. Link to a Decision when its reasoning helps.
...
- Look here when current work may need archived context.
...
- Archived material follows its destination's rules. Strip metadata that would keep its former behavioral role or claim current authority.
