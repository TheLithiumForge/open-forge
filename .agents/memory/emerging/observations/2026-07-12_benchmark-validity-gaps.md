...
- A developer-only `prepare` / `finalize` / `validate` / `render` runner now claims UUIDv4 run directories outside the source repository, snapshots and hashes ordered components plus CLI/prompt/rubric/spec inputs, records exact dirty-source Git evidence, and creates a post-composition worker Git baseline.
- Worker-visible composition rejects orchestration, `.git`, `node_modules`, CLI, prompt, and rubric leakage; exact collision overrides replace last-writer-wins behavior. Canary scans and initial/final tree manifests make contamination and drift observable.
- Preparation emits an owner token and external prepared-root digest. Keyed prepared and final seals authenticate evidence roots to the token holder, validation reconstructs result semantics rather than trusting stored conclusions, and a prepared-state check detects live workspace drift before a worker starts. The exact harness producer is now snapshotted and validation requires matching producer bytes.
- Real-path and topology guards cover the external runs root, experiment/run claims, workspace/evidence roots, component ancestry, and complete file-parent conflicts before protected writes or evidence claims proceed. `claim.json` binds the canonical runs-root/experiment/run path so copied or moved prepared runs cannot create a second finalization domain.
