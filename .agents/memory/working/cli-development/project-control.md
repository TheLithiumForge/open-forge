...
| -------------------------------------------------------- | --------------------------------------------------------------------------- | ------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | --------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| Task 14 “Extension Install”                              | [Extension Install](tasks/lifecycle/extension-install.md)                   | `ACTIVE`      | Not applicable   | Install reviewed Extension packages with isolated lifecycle, collision, dependency, and recovery integrity.         | First adoption command after integrated Task 12; immutable Green is in review and wider verification. | Streamlined assured task          | `codex/extension-install`; `/home/tedy/dev/open-forge-worktree/extension-install`; Green `9a6ae2fa`, tree `c39cfcb`    | Sol/xhigh Task Mastermind; one Brilliant Implementer; four fresh topic reviewers; Luna/max gate workers | Active isolated boundary; later squash integration follows complete acceptance.                               |
| Task 15 “Status”                                         | [Status](tasks/operations/status.md)                                        | `QUEUED`      | Not applicable   | Report typed facts for every producer present at its frozen baseline without diagnosis or mutation.                 | After Task 14; read-only contributor revalidation may overlap its immutable review.                   | Streamlined assured task          | Read-only preparation complete at `328599a0`; candidate revalidation active against Task 14 Green `9a6ae2fa`           | Sol/xhigh Task Mastermind supervising three bounded Luna/max inventories; no implementation             | Resolve three Gray authority gaps only after Task 14 acceptance and exact activation.                         |
...
| Task 7 “npm Package Manager Release and Local Linking”   | [npm Packages](tasks/delivery/01-npm-packages.md)                           | `QUEUED`      | Not applicable   | Extend the accepted package graph and local-link tooling to Linux, macOS, and Windows x64.                          | Read-only restart revalidation may overlap Task 14; implementation waits for its accepted boundary.   | Reopened streamlined task         | Platform-expansion phase 1/4, milestone 0/7 prerequisite audit active; exact mutating base remains deferred            | Sol/xhigh Task Mastermind supervising three bounded Luna/max inventories; no implementation             | Preserves the accepted 8/8 historical closeout and adds a separate 0/7 horizon.                               |
...
- Task 12 is Complete and dequeued at phase 5/5, milestone 6/6. Corrected source
  ...
  `9a6ae2fa`, tree `c39cfcb`, follows accepted Gray and Red and is now under
  four-topic whole-task review plus wider managed, Native AOT, and package
  verification. Its four accepted freezes and deferred, non-authoritative
  `.apm/**` idea remain recorded in its Task. Tasks 14–16 form the prioritized
  adoption slice. Task 15 is revalidating the candidate read-only; Task 7 is
  running a separate read-only restart audit and may implement beside Status
  only after Task 14 acceptance.
...
- Active tasks:
  - Task 14 “Extension Install” (phase 4/5): milestone 4/8 — immutable Green
    ...
    Release builds. Four ordinary topic reviewers are active under the Task
    Mastermind because the strict Review Mastermind runtime lacked its required
    named Git-object surface and launched no review unit.
- Active read-only prerequisites:
  - Task 15 “Status” remains queued while a Task Mastermind and three Luna/max
    inventories revalidate the Task 14 candidate; no Gray or implementation is
    authorized.
  - Task 7 “npm Package Manager Release and Local Linking” remains queued at
    platform-expansion phase 1/4, milestone 0/7 while a Task Mastermind and three
    Luna/max inventories audit reusable and rejected history; no package,
    linking, build, or source mutation is authorized.
...
- Next meaningful project boundary: disposition Task 14's four-topic review,
  complete wider managed, Native AOT, and package verification against immutable
  Green, apply at most one grouped correction, and accept or return the Task.
