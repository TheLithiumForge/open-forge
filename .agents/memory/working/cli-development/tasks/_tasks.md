...
- [ ] [Root Tooling Placement Remediation](root-tooling-placement.md) — Complete candidate at phase 5/5, milestone 8/8; final evidence passed and Overseer squash integration plus queue disposition remain — Implementer: Task 11 Task Mastermind with one Brilliant Implementer and one fresh review
...
- [Place live repository-owned agent tooling in explicit source scopes and remove the unexplained root scripts bucket](root-tooling-placement.md) - #Memory #Working #Contextual #Active #CLI #Task #Tooling #TypeScript #Testing
