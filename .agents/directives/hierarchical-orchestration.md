...
- Give top-level tasks stable numeric display positions. Use the exact status form `Task X/Y “<actual task name>” (phase A/B): milestone C/D`, with numeric progress and denominators in all four positions and the task's actual name in curly quotation marks. Keep optional local letter labels separate from this status and never treat them as task or phase identities. Fix denominators for the accepted horizon, never report regressing progress, and disclose a scope change as a new horizon instead of silently changing a denominator. End every in-progress Overseer commentary with this status line.
- Require Task, Integration, and Review Mastermind checkpoints to contain exactly `Done`, `Now`, `Next`, and `Blocker` lines. When a stable display mapping exists, start `Now` with the canonical status before naming the active operation.
