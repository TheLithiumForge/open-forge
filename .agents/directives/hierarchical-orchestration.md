...
- Under every rendered active task, list each active responsible agent as `<canonical agent name> — <agent type> (<repository-relative agent file>) — <exact model> / <reasoning effort>`. Include the Task Mastermind and current implementer, reviewer, or integration owner. Derive model and reasoning from runtime or invocation evidence; report `unreported` rather than infer missing metadata. Do not present completed helpers as active.
