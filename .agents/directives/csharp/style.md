...
- Declare a constant at the narrowest scope that owns it. A value used across the whole application belongs in one application-wide constants class; a value owned by one module or command belongs in that module's `<ModuleName>Constants`; a value used only inside one type is declared at the top of that type. Do not restate a constant that a wider scope already owns, and do not derive one by copying a literal that another constant already names. Distinct values that coincide today are not one constant: keep them separate when they answer different questions, such as a per-command schema version.
...
- Place a shared owner at the narrowest scope that covers every consumer **in the layer graph**, not in the folder tree. A helper consumed by an outer layer cannot live inside an inner one, however many inner consumers it also has: moving it there inverts a dependency that nothing but a boundary check will catch. Before extracting a duplicate into a shared type, name its consumers and place it above all of them.
