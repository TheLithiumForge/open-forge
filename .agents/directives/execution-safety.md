...
- Configure shell permissions from a default `ask` or `deny` posture. Do not claim local-only enforcement from an allow-all command rule plus a blacklist: unmatched shells, interpreters, aliases, Git plumbing, and platform-specific commands would remain an escape path. Permit a command only through an exact reviewed rule or one explicit approval for the displayed action.
