...
9. **Pending: rename guidelines to guidance.** Decide whether `guidelines/`, `_guidelines.md`, and #Guideline become `guidance/`, `_guidance.md`, and #Guidance. If approved, update governance descriptors, installed payload files, README structure, generated indexes, CLI tests, and migration/update behavior before user documentation is finalized.
10. **Active review: implement guidance file by file.** The current guidelines payload governor and minimum `_guidelines.md` are implemented for review under the old name. After priority 9, review them under the final primitive name. They define selective routing, contextual judgment, reasoned adaptation, and recursive scope without seeded guidance or cross-primitive rules.
...
17. **Active review: create Memory implementation files.** The installable payload files for `memory/`, `working/`, `emerging/`, `crystallized/`, and `archived/` are drafted for review, including their minimum `_{category}.md` entrypoints and generated regions.
18. **Define slugified category and route naming.** Decide the slug contract for user-created and extension-created category folders; keep canonical framework roots pinned; prefer stable lowercase kebab-case for slug segments; keep display names in titles, descriptions, or frontmatter; define collision handling; ensure existing user paths are not renamed without explicit approval; and decide how filesystem slugs interact with extension/template manifests.
19. **Define route templates for flexible scoped extras.** Define pinned routes versus movable route templates; decide template notation such as `[project]`, `[scope]`, and `[state]`; decide how extras such as documents, references, decisions, archives, projects, tasks, backlog, and workflow outputs can be installed globally, project-scoped, custom-scoped, or under any owning route; require scaffold metadata for missing ancestor categories; and preserve user choice without creating vague default buckets.
...
23. **Explore extensions, scoped containers, and CLI experience.** After the #Core framework and trust model are stable, define extension discovery, extension payloads, category scaffolding, route-template installation, extension authoring, scoped decision/archive guidance, previews, installation, ownership, updates, removal, provenance, compatibility, and an optional interactive interface.

## Current Focus

Priorities 1 through 8 are approved and implemented. Priority 9 is pending. Priority 10 is implemented under the current `guidelines/` name and remains affected by the pending guidance naming decision. Priorities 11 and 12 have approved governance and pending implementation wording review. Priorities 14 through 17 are drafted for Memory review. Priorities 13 and 18 through 23 are pending.
