...
11. **Active review: implement skills file by file.** The skills payload governor and minimum `_skills.md` are implemented for review. They preserve established runtime ownership, define bounded reusable capabilities, and route scoped skills without seeded skill content.
...
17. **Active review: create Memory implementation files.** The installable payload files for `memory/`, `working/`, `emerging/`, `crystallized/`, `archived/`, and the first universal child routes are drafted for review. The `observations/` route now emphasizes agent-written post-work findings while keeping observations contextual. Memory child routes now state promotion or extraction paths explicitly enough to cover movement inside #Memory and extraction into matching #Core routes without relying only on root-level inference.
...
Priorities 1 through 10 are approved and implemented. Priority 11 is active with a skills wording pass applied for review. Priority 12 has approved governance and pending implementation wording review. Priorities 14 through 17 are drafted for Memory review with the latest child-route wording pass applied. Priorities 13 and 18 through 24 are pending.
