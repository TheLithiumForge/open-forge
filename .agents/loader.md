...
- A user-owned `{name}.overwrite.md` is not an independent route; when its `{name}.md` base loads, read the companion immediately afterward
- The overwrite inherits the base route, scope, and loading behavior, is not independently indexed or selected, and has final precedence within that file's scope
