...
Agents load emerging memory selectively and keep unrelated candidate material out of current work.
...
Layer 3 workflows may use emerging memory for useful material that is not ready to become current truth.
...
- promotes operational emerging material into matching `#Core` routes, including user-created `#Core` categories and files
