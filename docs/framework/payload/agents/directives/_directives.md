...
Generated `entries` list direct directive files and direct child directive categories. Direct file entries expose their authored #LoadNow tag. The [routed Markdown contract](../../../../../.agents/memory/crystallized/documents/framework/markdown/routes.md) defines metadata, `entry` representation, entrypoint naming, and marker shape. The routing model governs recursive discovery and generation.
