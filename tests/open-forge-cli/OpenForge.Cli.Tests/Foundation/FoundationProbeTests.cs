...
public sealed class FoundationProbeTests
{
    ...
    public async Task ProvesFoundationBoundaries()
    {
        ...
        Assert.Equal(expectedBytes, await File.ReadAllBytesAsync(Path.Combine(workspace.Path, "foundation-probe.md"), TestContext.Current.CancellationToken));
        Assert.Equal(
    ...
    public async Task ReportsNonDirectoryWorkspace()
    {
        ...
        Assert.Equal("occupied", await File.ReadAllTextAsync(filePath, TestContext.Current.CancellationToken));
    }
    ...
    private sealed class TemporaryWorkspace : IDisposable
    {
        ...
        {
            var path = System.IO.Path.Combine(System.IO.Path.GetTempPath(), $"open-forge-foundation-{Guid.NewGuid():N}");
