...
internal static class CliTerminalInputValidator
{
        ...
        CliGlobalOptionSymbols options)
    {
        return ReferenceEquals(option, options.Workspace)
            || ReferenceEquals(option, options.Format)
            || ReferenceEquals(option, options.Detail)
            || ReferenceEquals(option, options.DetailFilter)
    ...
    private static CliInvalidInput Invalid()
    {
        return new CliInvalidInput(
            ...
            [global::OpenForge.Cli.OutputText.Shared.SharedText.MessageTerminalOptionsCannotBeCombinedWithCommandInput()]);
    }
}
