...
public sealed class FindUniverseResolverRedTests
{
    ...
    public void SelectorsPreserveFormsKindsExpansionsAndTypedOutcomes(string scenario)
    {
        var expectations = scenario switch
        {
            "resolved" => new[]
            {
            ...
            _ => throw new ArgumentOutOfRangeException(nameof(scenario), scenario, "The Find selector scenario is not defined."),
        };
    ...
    [InlineData("union-order")]
    [InlineData("exclusion-wins")]
    [Trait("Feature", "find-query"), Trait("Evidence", "Unit")]
    public void IncludeExcludeAlgebraIsDeduplicatedOrderIndependentAndExclusionFirst(string scenario)
    {
        var expectations = scenario switch
        {
            "union-order" => new[]
            ...
            _ => throw new ArgumentOutOfRangeException(nameof(scenario), scenario, "The Find algebra scenario is not defined."),
        };
    ...
    [InlineData("entrypoint-scope")]
    [InlineData("narrow-sources")]
    [Trait("Feature", "find-query"), Trait("Evidence", "Unit")]
    public void EffectiveSelectionProjectsOnlyInScopeCandidatesAndIssues(string scenario)
    {
        var expectations = scenario switch
        {
            "entrypoint-scope" => new[]
            {
            ...
            _ => throw new ArgumentOutOfRangeException(nameof(scenario), scenario, "The Find scope scenario is not defined."),
        };
    ...
    [InlineData("root-boundaries")]
    [InlineData("candidate-boundaries")]
    [InlineData("identity-and-pairing")]
    [InlineData("aliases")]
    [Trait("Feature", "find-query"), Trait("Evidence", "Unit")]
    public void CatalogueIssuesMapToExactFindFindingsAndCandidateCounts(string scenario)
    {
        var expectations = scenario switch
        {
            "root-boundaries" => new[]
            ...
            _ => throw new ArgumentOutOfRangeException(nameof(scenario), scenario, "The Find catalogue-issue scenario is not defined."),
        };
        ...
        IEnumerable<string> exclude)
    {
        ...
        var request = new FindRequest(
            ...
            query,
            presentation);
        var sourceContext = new FindSourceReadContext(
            fixture.Catalogue,
            new SourceDocumentReader(fixture.Workspace),
            fixture.DefaultScope);
        return new FindUniverseInput(request, sourceContext);
    }
    ...
    private sealed record AlgebraExpectation(
        string[] Include,
        string[] Exclude,
        string[] ExpectedPaths);

    private sealed record ScopeExpectation(
        string Selector,
        string[] ExpectedCandidatePaths,
        string[] ExpectedIssuePaths);

    private sealed record CatalogueIssueExpectation(
