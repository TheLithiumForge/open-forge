...
namespace OpenForge.Cli.Core.UnitTests.Commands.Library.Sync.Shared.Completion;

public sealed class LibrarySyncCompletionTests
{
    ...
    {
        var input = LibraryMutationCompletionData.Sync();
        ...
        var result = LibrarySyncCompletion.Complete(input);
        ...
        Assert.Contains(result.Result.Findings, finding => finding.Code == (
            preparation.State == RecoveryBundlePreparationState.Cancelled ? LibrarySyncFindingCode.Interrupted : LibrarySyncFindingCode.RecoveryUnavailable));
        if (hasResidual)
        {
    ...
    {
        var input = LibraryMutationCompletionData.Sync();
        var plan = Assert.IsType<LibrarySyncPlan>(input.Plan);
        var unavailable = new LibrarySyncFinding
        {
            Code = LibrarySyncFindingCode.RecordUnavailable,
        ...
        var unsafeTarget = new LibrarySyncFinding
        {
    ...
    {
        var input = LibraryMutationCompletionData.Sync();
        var observations = Assert.IsType<LibrarySyncPlanningInput>(input.Observations);
        ...
        input = input with
        {
            ...
            Plan = Assert.IsType<LibrarySyncPlan>(input.Plan) with { Input = observations },
            Execution = LibraryMutationCompletionData.Empty(),
        };
        var result = LibrarySyncCompletion.Complete(input);
