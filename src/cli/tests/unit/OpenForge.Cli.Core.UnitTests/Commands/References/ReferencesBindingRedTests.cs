...
public sealed class ReferencesBindingRedTests
{
    ...
    public async Task ClosedBindingInvokesOperationOnceAndSelectsOneRenderer()
    {
        ...
        var operation = new ReferencesOperation(
            new ReferencesOperationComponents
            {
                ...
                SourceReferenceResolver = new SourceReferenceResolver(physicalPathResolver),
                UniverseFilterResolver = new SourceUniverseFilterResolver(
                    new SourceReferenceResolver(physicalPathResolver)),
                LayerReader = (_, _, _) => throw new InvalidOperationException("layer read must not run"),
                MarkdownParser = _ => throw new InvalidOperationException("markdown parse must not run"),
                PhysicalPathResolver = (_, _) => throw new InvalidOperationException("path resolve must not run"),
                StrictUtf8Reader = (_, _, _) => throw new InvalidOperationException("utf8 read must not run"),
