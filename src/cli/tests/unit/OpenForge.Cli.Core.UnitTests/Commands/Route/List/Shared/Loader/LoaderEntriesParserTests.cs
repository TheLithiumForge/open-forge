...
public sealed class LoaderEntriesParserTests
{
    ...
    {
        var result = LoaderEntriesParser.Parse(LoaderContents(
            "\n- [Second](second/_second.md) - #Second\n"
            + "- [First](first/_first.md) - #First\n",
            "## Axioms\n\n- inherited - No local axioms.\n\n"));

        Assert.Equal(LoaderEntriesParseState.Valid, result.State);
        Assert.Equal(
        ...
        Assert.All(result.Destinations, destination =>
            Assert.Equal(LoaderDestinationParseState.Valid, destination.State));
    ...
    {
        var result = LoaderEntriesParser.Parse(LoaderContents(markerBody));
    ...
    public void EntriesParserRetainsUnsafeDestinationResults()
    {
        ...
        Assert.Equal(LoaderDestinationParseState.Unsafe, destination.State);
    ...
    public void EntriesParserRejectsMalformedSections(string loaderContents)
    {
        ...
        Assert.Equal(LoaderEntriesParseState.Malformed, result.State);
