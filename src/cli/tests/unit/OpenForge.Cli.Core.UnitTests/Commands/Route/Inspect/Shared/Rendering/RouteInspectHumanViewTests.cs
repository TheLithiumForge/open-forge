...
{
    [Theory(DisplayName = "Route Inspect views show incomplete conditions before the profile and preserve JSON"), Trait("Feature", "route-inspect"), Trait("Evidence", "Unit")]
    ...
    public void ConditionsPrecedeProfile(int view)
    {
        ...
        var jsonRequest = RouteInspectPresentationTestData.Presentation(result, CliView.Expanded, CliOutputFormat.Json);
        var before = RouteInspectJsonRenderer.Render(jsonRequest);
        var output = RouteInspectHumanRenderer.Render(RouteInspectPresentationTestData.Presentation(result, (CliView)view));

        var condition = Assert.Single(result.Conditions);
        Assert.True(output.IndexOf(condition.MachineCode, StringComparison.Ordinal) < output.IndexOf("Where this source belongs", StringComparison.Ordinal));
        Assert.Contains("Next: open-forge doctor", output, StringComparison.Ordinal);
        Assert.Equal(1, output.Split("Own source:", StringSplitOptions.None).Length - 1);
        Assert.Equal(before, RouteInspectJsonRenderer.Render(jsonRequest));
    }
