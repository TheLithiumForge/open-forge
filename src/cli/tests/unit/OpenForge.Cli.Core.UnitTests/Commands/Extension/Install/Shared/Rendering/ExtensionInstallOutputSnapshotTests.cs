...
public sealed class ExtensionInstallOutputSnapshotTests
{
    [Trait("Boundary", "Output")]
    [Fact(DisplayName = "Extension Install minimal text publication matches its reviewed snapshot")]
    public void MinimalText() => Render(CliFormat.Text, CliDetail.Minimal).AssertSnapshot();

    [Trait("Boundary", "Output")]
    [Fact(DisplayName = "Extension Install standard text publication matches its reviewed snapshot")]
    public void StandardText() => Render(CliFormat.Text, CliDetail.Standard).AssertSnapshot();

    [Trait("Boundary", "Output")]
    [Fact(DisplayName = "Extension Install minimal JSON publication matches its reviewed snapshot")]
    public void MinimalJson() => Render(CliFormat.Json, CliDetail.Minimal).AssertSnapshot();

    [Trait("Boundary", "Output")]
    [Fact(DisplayName = "Extension Install standard JSON publication matches its reviewed snapshot")]
    ...
    private static string Render(CliFormat format, CliDetail view)
    {
        ...
        var facts = new ExtensionInstallResultFacts
        {
            ...
            Recovery = new(ExtensionInstallRecoveryState.Retained, [".agents/open-forge.lock.json", ".agents/protected.md"], "/recovery/bundle"),
