...
public sealed class MarkdownDocumentParserTests
{
    ...
    {
        const string source = "Visible `code` [label](target.md)\n";

        var facts = new MarkdownDocumentParser().Parse(source);

        Assert.NotEmpty(facts.VisibleText);
        var code = Assert.Single(facts.OpaqueSpans, span => span.IsCode);
        Assert.Equal(SpanOf(source, "`code`"), code.Span);
