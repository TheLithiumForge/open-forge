...
public sealed class PublishedLibraryAttachProcessTests
{
    ...
    public async Task DryRunReportsCompletePlanWithoutEffects()
    {
        ...
        using var document = PublishedLibraryWorkspace.Result(
            await workspace.ReadOnlyAsync(PublishedExecutableTarget.Discover(), "library", "attach", "team-knowledge", "shared/team-knowledge", "--dry-run", "--format=json"), "completed");
        var result = document.RootElement.GetProperty("data");
        Assert.Equal("dry-run", result.GetProperty("mode").GetString());
        Assert.Equal("shared/team-knowledge", result.GetProperty("sourceFolder").GetString());
        Assert.Equal(".", result.GetProperty("destinationFolder").GetString());
    ...
    [Fact, Trait("Feature", "library-mutation"), Trait("Evidence", "EndToEnd")]
    public async Task ApplyCreatesExactRelativeProjectionAndOwnershipReceipt()
    {
        using var workspace = new PublishedLibraryWorkspace();
        ...
        {
            using var document = PublishedLibraryWorkspace.Result(
                await workspace.RunAsync(PublishedExecutableTarget.Discover(), "library", "attach", "team-knowledge", "shared/team-knowledge", "--to", "docs", "--automatic", "--format=json"), "completed");
            ...
            Assert.Equal(["$schema", "schemaVersion", "extensions", "libraries"], [.. record.RootElement.EnumerateObject().Select(property => property.Name)]);
            ...
            Assert.True(document.RootElement.GetProperty("data").GetProperty("recorded").GetBoolean());
    ...
    public async Task OccupiedDestinationBlocksWholeAttach()
    {
        ...
        using var document = PublishedLibraryWorkspace.Result(
            await workspace.ReadOnlyAsync(PublishedExecutableTarget.Discover(), "library", "attach", "team-knowledge", "shared/team-knowledge", "--format=json"), "blocked", 5);
