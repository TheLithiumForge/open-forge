...
public sealed class RouteRemoveRecoveryIdentityIntegrationTests
{
    ...
    {
        using var workspace = RouteRemoveIntegrationWorkspace.Create("route-remove-recovery-identity");
        ...
        try
        {
            ...
            var result = await RouteRemoveRecoveryLifecycle.DeleteExactAsync(
                new RouteRemoveRecoveryDeletionInput
                {
                    Held = new RouteRemoveHeldApplication(Plan: plan, OperationId: operationId, Lease: lease),
            ...
            if (changed)
            {
                ...
                Assert.Equal(RouteRemoveRecoveryState.Unknown, result.Recovery.State);
                Assert.Equal(RouteRemoveFindingCode.RecoveryFailed, result.Finding?.Code);
            ...
            else
            {
                ...
                Assert.Equal(RouteRemoveRecoveryState.Removed, result.Recovery.State);
                Assert.Null(result.Finding?.Code);
                Assert.Null(result.Recovery.ResidualPath);
                Assert.Empty(result.Recovery.ProtectedPaths);
            }
        }
        finally
        {
