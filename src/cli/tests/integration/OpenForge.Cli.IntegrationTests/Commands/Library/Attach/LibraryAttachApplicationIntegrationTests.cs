...
public sealed class LibraryAttachApplicationIntegrationTests
{
    [Trait("Boundary", "OS")]
    ...
    public static async Task HeldLeaseRevalidatesWholePlanAndRetainsRealExecutionReceipts(string scenario)
    {
        ...
        using var locks = WorkspaceLockTestStore.Create("library-attach-application");
        workspace.Source();
        ...
        var recovery = RecoveryBundleInput.Create(workspace.Workspace, command: "library attach",
            attribution: RecoveryBundleAttribution.Create(RecoveryBundleProducer.Library, RecoveryBundleOperation.Attach, workspace.Workspace),
            ...
            [
                ...
                RecoveryBundleTarget.CreateReversible(recordChange, recordBefore),
            ]);
        ...
        try
        {
            ...
            {
                workspace.Write(LibraryMutationWorkspace.RecordPath, "Changed local record occupant.");
            }
            ...
            var execution = await LibraryAttachApplication.ApplyAsync(new LibraryAttachApplicationInput
    ...
    private static LibraryAttachPlan Plan(LibraryMutationWorkspace workspace)
    {
        var record = LibraryMutationApplicationData.ReadRecord(workspace, exists: false, registered: false);
        var before = Assert.IsType<FileStateSnapshot>(record.Snapshot);
        var input = new LibraryAttachPlanningInput
        {
            Request = workspace.Attach(LibraryMode.Apply),
            Record = record,
        ...
        return new LibraryAttachPlan
        {
            ...
            GeneratedRegions = [],
            OwnershipChange = PlannedFileChange.Create(before.Expectation, intendedBytes),
