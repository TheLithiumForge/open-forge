...
{
    [Fact]
    public async Task MixedSyncQuestionDisclosesCreationRetentionAndRetirement()
    {
        ...
        var result = await new LibrarySyncOperation(permissions).ExecuteAsync(workspace.Sync(LibraryMode.Apply) with { AllowPrompt = true },
            TestContext.Current.CancellationToken);
        Assert.Equal(CliSemanticStatus.Blocked, result.Status);
        Assert.Contains("docs/new.md (create link)", output.ToString(), StringComparison.Ordinal);
        Assert.Contains("docs/kept.md (retain link)", output.ToString(), StringComparison.Ordinal);
        Assert.Contains("docs/old.md (remove link)", output.ToString(), StringComparison.Ordinal);
    ...
    public async Task RememberedDirectoryCoversNewDescendantAndRetirementWithoutTouchingContent()
    {
        ...
        workspace.Write(".agents/open-forge.permissions.json", """
            ...
            """);
        var permissionBefore = File.ReadAllBytes(workspace.Absolute(".agents/open-forge.permissions.json"));
        ...
        try
        {
            ...
            Assert.Equal(permissionBefore, File.ReadAllBytes(workspace.Absolute(".agents/open-forge.permissions.json")));
    ...
    public async Task UncoveredSiblingBlocksWholeSyncAndProposesOnlyItsParent()
    {
        ...
        workspace.Write(".agents/open-forge.permissions.json", """
            {"schemaVersion":1,"extensions":[],"libraries":[{"id":"team-knowledge","sourceRoot":"shared/team-knowledge","paths":[],"directories":["docs/approved"]}]}
            """);
        var before = workspace.Snapshot();
        ...
        Assert.Equal("docs/new/b.md", Assert.Single(result.Result.Permissions.Missing).Path);
