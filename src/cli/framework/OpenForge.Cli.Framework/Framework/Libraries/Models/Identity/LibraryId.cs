...
internal sealed record LibraryId
{
    ...
    /// <summary>
    /// The identifier for <paramref name="value"/>, or null when it is not a well
    /// formed Library ID. A binder reads an unvalidated operand and reports the
    /// refusal through its own findings, so it asks rather than catching.
    /// </summary>
    internal static LibraryId? TryCreate(string? value)
    {
        try
        {
