...
{
    ValueTask<LibraryStatusView> ReadStatusAsync(CliWorkspace workspace, WorkspaceOwnershipRead ownership, CancellationToken cancellationToken);
...
internal sealed class LibraryOperationalContributor : ILibraryOperationalContributor
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (record.State == LibraryRegistrationReadState.Complete && record.Record is { } value)
            {
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            var ownership = await WorkspaceOwnershipReader.ReadAsync(resolver, workspace, cancellationToken)
    ...
    {
        if (record.OwnershipObservation is null && record.State is (LibraryRegistrationReadState.Malformed or LibraryRegistrationReadState.Blocked)
        ...
        if (record.OwnershipObservation is null && record.State == LibraryRegistrationReadState.Unavailable
    ...
    private static LibraryRegistrationRead InterruptedRecord(CliWorkspace workspace)
        => new()
        {
