...
internal sealed class LibraryInventoryCollector
{
    ...
    private void ObserveChild(string child, CancellationToken cancellationToken)
    {
        ...
        catch (IOException exception) when (IsWindowsSharingDenial(exception))
        {
            Unavailable(path, "The eligible Library source file is unavailable because another process is using it.");
            return;
        }
        catch (IOException)
        {
