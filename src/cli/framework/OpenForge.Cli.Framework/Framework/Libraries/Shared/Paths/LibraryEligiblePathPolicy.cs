...
internal static class LibraryEligiblePathPolicy
{
        ...
        out LibraryInventoryExclusionKind kind)
    {
        ...
        var agentsIndex = Array.FindIndex(originalSegments, segment =>
            string.Equals(segment, WorkspaceSettingsDefinitions.DirectoryName, StringComparison.Ordinal));
        if (agentsIndex < 0)
        {
            kind = default;
            return false;
        ...
        if (originalPath is WorkspaceOwnershipDefinitions.RelativePath
