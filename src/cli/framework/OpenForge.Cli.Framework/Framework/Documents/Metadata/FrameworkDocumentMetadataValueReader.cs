...
internal static class FrameworkDocumentMetadataValueReader
{
    internal static bool TryRead(
        ...
        out string? observedDescription,
        out IReadOnlyList<string> observedTags,
        out bool malformed)
    {
        metadata = null;
        tagSpans = [];
        ...
        if (descriptionNode?.Scalar is { } descriptionScalar
            && !string.IsNullOrWhiteSpace(descriptionScalar.Value))
        {
            observedDescription = descriptionScalar.Value;
        }
        ...
        var responsibilityMissing = false;
        ...
        observedTags = tagFacts?.Select(tag => tag.Value).ToArray() ?? [];
        if (responsibilityMissing)
        {
            return false;
        }
