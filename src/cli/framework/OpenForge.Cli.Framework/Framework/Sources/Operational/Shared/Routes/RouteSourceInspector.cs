...
internal sealed class RouteSourceInspector
{
            ...
            CancellationToken cancellationToken)
    {
        ...
        return new RouteWorkspaceEntryObservation(
            new RouteSourceLayerObservation(path, read.State, read.Value, read.Failure?.DirectCause),
            state);
    }
    ...
    private static RouteSourceLayerObservation Layer(SourceDocumentReadResult read)
        => new(
            ...
            read.Read?.Value,
            read.Verification.Failure?.DirectCause ?? read.Read?.Failure?.DirectCause);
}
