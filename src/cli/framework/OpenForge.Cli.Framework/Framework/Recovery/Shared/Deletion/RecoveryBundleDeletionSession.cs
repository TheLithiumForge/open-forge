...
internal sealed class RecoveryBundleDeletionSession
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (frozenCatalogue.State != RecoveryBundleCatalogueState.Available
            ...
            || frozenCatalogue.Candidates.Any(candidate => !IsCatalogueMember(candidate, lease)))
        {
    ...
    private static bool IsCatalogueMember(RecoveryBundleCandidateSnapshot candidate, WorkspaceLockLease lease)
        => RecoveryBundleDeletionValidation.BelongsToWorkspace(candidate, lease)
            && RecoveryBundleFormatV1.TryParseCandidateFileName(Path.GetFileName(candidate.Path), out _, out var kind)
            && candidate.Kind == kind
            && (((candidate.Kind, candidate.Integrity) is
                ...
                || candidate.Kind == RecoveryBundleCandidateKind.Final
                    && candidate.Integrity == RecoveryBundleIntegrity.Malformed
                    && ReadFileBoundaryFailure(candidate) is null);
