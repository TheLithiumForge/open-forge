...
internal static class StatusStructureAggregator
{
        ...
        OperationalInstallationState installation)
    {
        ...
        return new StatusStructure
        {
            ...
            {
                Count = StatusMeasurementCalculator.Value(StatusStateMap.Value(valueState), routes.CurrentRootCategories.Count),
                Added = added,
                Removed = removed,
            },
    ...
    internal static StatusStructure Unavailable()
        => new()
        {
            RootCategories = new StatusRootCategories
            {
                Count = new StatusIntegerValue(StatusValueState.Unavailable, null),
                Added = [],
                Removed = [],
            },
            GeneratedNavigation = [],
        ...
        OperationalInstallationState installation)
    {
        var state = installation == OperationalInstallationState.Uninstalled
            ? StatusGeneratedNavigationState.NotApplicable
            : StatusStateMap.GeneratedNavigation(target.State);
