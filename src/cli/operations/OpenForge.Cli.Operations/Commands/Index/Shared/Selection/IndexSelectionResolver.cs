...
internal sealed class IndexSelectionResolver
{
        ...
        GeneratedNavigationFormation formation)
    {
        ...
        for (var index = 0; index < request.SourceReferences.Count; index++)
        {
            ...
            var finding = ResolutionFinding(resolution, index + 1, formation);
            if (resolution.Form == SourceReferenceKind.SourcePath
                && resolution.CanonicalPath is { } operand
                && SourceLogicalPath.IsCanonicalSource(operand))
            {
                var directorySources = formation.Catalogue.Sources.Where(source =>
                    SourceFormClassifier.IsEntrypoint(source.Base.Form)
                    && SourceLogicalPath.ReadParent(source.Identity.CanonicalBasePath) == operand).ToArray();
                if (directorySources.Length > 0)
                {
                    finding = finding with
                    {
                        Details = finding.Details! with
                        {
                            InputProblem = IndexInputProblem.Folder,
                            CorrectedSourceId = directorySources.Length == 1 ? directorySources[0].Identity.AutomaticId : null,
                        },
                    };
                }
            }
        ...
        GeneratedNavigationFormation formation)
    {
                ...
                formation: formation)))
        {
            ...
            {
                Operand = resolution.Value,
                InputProblem = resolution.State switch
                {
                    SourceReferenceResolutionState.Unknown when resolution.Form == SourceReferenceKind.SourceId => IndexInputProblem.UnknownId,
                    SourceReferenceResolutionState.Unknown => IndexInputProblem.UnknownPath,
                    SourceReferenceResolutionState.Unsupported => IndexInputProblem.Unsupported,
                    _ => IndexInputProblem.Invalid,
                },
            },
        };
    }
