...
internal sealed class IndexProjectionBuilder
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var metadataFindings = new List<IndexFinding>();
            ...
            foreach (var child in ReadDirectChildren(context.Formation, target))
            {
                ...
                if (!childDocument.IsComplete)
                {
                    findings.Add(CreateAcquisitionFinding(
                        ...
                        child,
                        context.Formation) with
                    { Details = new IndexFindingDetails { ParentPath = targetPath } });
                    targetAvailable = false;
                    continue;
                }

                var documentFacts = childDocument.ReadDocument();
                var metadataFacts = _metadataParser.Parse(documentFacts, child.Base.Form);
                metadata.Add(child.Identity.CanonicalBasePath, new GeneratedNavigationMetadata(child, metadataFacts));
                if (metadataFacts.State != SourceAuthoredMetadataState.Complete)
                {
                    var unclosed = documentFacts.Frontmatter.State == MarkdownFrontmatterState.Unavailable;
                    var missing = metadataFacts.State is SourceAuthoredMetadataState.Missing or SourceAuthoredMetadataState.NotApplicable;
                    var optional = metadataFacts.State == SourceAuthoredMetadataState.Missing
                        && IsOptionalMetadataSource(child.Base.Form);
                    metadataFindings.Add(new IndexFinding(
                        optional
                            ? IndexFindingCode.MetadataOptional
                            : missing ? IndexFindingCode.MetadataIncomplete : IndexFindingCode.MetadataUnsafe,
                        sourceOccurrence: null,
                        source: IndexLogicalSourceProjector.Project(child, context.Formation),
                        cause: unclosed ? "The frontmatter block is not closed." : missing
                            ? optional ? "Optional authored metadata is missing." : "Required authored metadata is missing."
                            : child.Base.Form == SourceDocumentForm.Skill
                                ? "The native Skill frontmatter is invalid."
                                : "The authored metadata is invalid.",
                        candidates: [])
                    {
                        Details = new IndexFindingDetails
                        {
                            ParentPath = targetPath,
                            MetadataProblem = unclosed ? IndexMetadataProblem.UnclosedFrontmatter
                                : missing ? IndexMetadataProblem.Missing : IndexMetadataProblem.Invalid,
                            Line = unclosed ? 1 : null,
                            Column = unclosed ? 1 : null,
                        },
                    });
                }
            }
    ...
    internal IndexProjectionFormation Form(IndexProjectionAssembly assembly)
    {
        ...
        var findings = assembly.Readiness.Findings.Concat(assembly.MetadataFindings).ToList();
        ...
        foreach (var region in projection.Regions.Where(region =>
                     ...
                     && !findings.Any(finding => finding.Details?.ParentPath == region.CanonicalPath)))
        {
        ...
        var finalFindings = ConvertIndependentMetadataFindings(
            DeduplicateFindings(findings),
            projection,
            assembly);
    ...
    private static IReadOnlyList<IndexFinding> ConvertIndependentMetadataFindings(
        IReadOnlyList<IndexFinding> findings,
        GeneratedNavigationProjection projection,
        IndexProjectionAssembly assembly)
    {
        if (!assembly.Selection.IsComplete
            || assembly.Readiness.Findings.Count != 0
            || assembly.Readiness.UnavailableTargetPaths.Count != 0)
        {
            return findings;
        }

        var unavailableRegions = projection.Regions
            .Where(region => region.State == GeneratedNavigationRegionState.Unavailable)
            .ToDictionary(region => region.CanonicalPath, StringComparer.Ordinal);
        var metadataByPath = assembly.Metadata
            .ToDictionary(
        ...
        if (!projection.Regions.Any(region => region.State == GeneratedNavigationRegionState.Available
                && region.Change?.Kind == GeneratedNavigationChangeKind.Update))
        {
            return findings;
        }

        var nonOptionalFindings = findings
            .Where(finding => finding.Code != IndexFindingCode.MetadataOptional)
            .ToArray();
        if (nonOptionalFindings.Length == 0
            || nonOptionalFindings.Any(finding => !IsSkippableMetadataFinding(
                finding,
                unavailableRegions,
                metadataByPath)))
        {
        ...
        var findingPaths = nonOptionalFindings
            .Select(finding => finding.Details?.ParentPath)
            .Where(path => path is not null)
            .Cast<string>()
            .ToHashSet(StringComparer.Ordinal);
        if (!findingPaths.SetEquals(unavailableRegions.Keys))
        {
            return findings;
        }

        return findings
            .Select(finding => finding.Code == IndexFindingCode.MetadataUnsafe
                ? new IndexFinding(
                    code: IndexFindingCode.MetadataSkipped,
    ...
    private static bool IsSkippableMetadataFinding(
        IndexFinding finding,
        IReadOnlyDictionary<string, GeneratedNavigationRegion> unavailableRegions,
        IReadOnlyDictionary<string, GeneratedNavigationMetadata> metadataByPath)
    {
        if (finding.Code != IndexFindingCode.MetadataUnsafe
            || finding.Details is not
            {
                ParentPath: { } parentPath,
                MetadataProblem: { } metadataProblem
            }
            || !unavailableRegions.TryGetValue(parentPath, out var region)
            || region.UnavailableReason is not (GeneratedNavigationRegionUnavailableReason.MetadataInvalid
                or GeneratedNavigationRegionUnavailableReason.MetadataUnavailable)
            || finding.Source is not { } source
            || !metadataByPath.TryGetValue(source.Path, out var metadata)
            || !IsIndependentSkippableMetadataSource(metadata.Source.Base.Form)
            || (metadataProblem == IndexMetadataProblem.Missing
                && !IsOptionalMetadataSource(metadata.Source.Base.Form)))
        {
            return false;
        }

        return metadataProblem is IndexMetadataProblem.Missing
            or IndexMetadataProblem.Invalid
            or IndexMetadataProblem.UnclosedFrontmatter;
    }

    private static bool IsOptionalMetadataSource(SourceDocumentForm form)
        => form == SourceDocumentForm.Markdown || SourceFormClassifier.IsEntrypoint(form);

    private static bool IsIndependentSkippableMetadataSource(SourceDocumentForm form)
        => IsOptionalMetadataSource(form) || form == SourceDocumentForm.Skill;
