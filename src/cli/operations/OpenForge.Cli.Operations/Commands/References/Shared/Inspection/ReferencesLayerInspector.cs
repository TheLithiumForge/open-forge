...
internal sealed class ReferencesLayerInspector
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            if (direction == ReferencesDirection.In && DescribesLinkTarget(finding.Code))
            {
                continue;
            }
    ...
    /// <summary>
    /// Whether a finding describes where a link points rather than the file that holds it.
    /// Those facts belong to the outgoing view of the source that owns the link. The incoming
    /// scan reads every source looking for links into the selected one, so recording them here
    /// would report the selected source's own links a second time, and would answer "what links
    /// in" with the link health of unrelated files.
    /// </summary>
    private static bool DescribesLinkTarget(ReferencesFindingCode code) => code switch
    {
        ReferencesFindingCode.DestinationMalformed
            ...
            or ReferencesFindingCode.TargetAmbiguous
            or ReferencesFindingCode.TargetUnreadable
            or ReferencesFindingCode.LinkEncodingInvalid => true,
        _ => false,
    };
