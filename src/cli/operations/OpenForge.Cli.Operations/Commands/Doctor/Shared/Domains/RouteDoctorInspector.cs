...
internal static class RouteDoctorInspector
{
    ...
    internal static DoctorDomainReport Inspect(RouteDoctorView view)
    {
        ...
        var unavailable = view.Routes.RouteFacts.Any(fact => fact.State == SourceRouteState.Unavailable)
            ...
            || view.Sources.Any(source => !source.IsReadable)
            || view.GeneratedNavigation.Any(item =>
                (item.State is OperationalGeneratedNavigationState.Unavailable
                    or OperationalGeneratedNavigationState.Blocked)
                && !IsMetadataOnlyUnavailable(view, item))
    ...
    private static bool IsMetadataOnlyUnavailable(
        RouteDoctorView view,
        DoctorGeneratedNavigationTargetObservation observation)
        => view.State == OperationalViewState.Complete
            && view.Sources.All(source => source.IsReadable)
            && observation.State == OperationalGeneratedNavigationState.Unavailable
            && (observation.UnavailableReason is GeneratedNavigationRegionUnavailableReason.MetadataInvalid
