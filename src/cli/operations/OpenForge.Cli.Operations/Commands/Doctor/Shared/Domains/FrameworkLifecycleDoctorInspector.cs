...
internal static class FrameworkLifecycleDoctorInspector
{
        ...
        IReadOnlySet<string> currentNavigationPaths)
    {
        ...
        else
        {
            ...
            var hasNonNavigationDivergence = false;
            foreach (var target in view.Targets)
            {
                ...
                if (IsNonNavigationDivergence(target, view.SourceAvailability))
                {
                    hasNonNavigationDivergence = true;
                }
    ...
    private static void AddManagedSet(
        bool isMixed,
        bool hasNonNavigationDivergence,
        ICollection<DoctorFinding> findings)
    {
        if (isMixed && hasNonNavigationDivergence)
        {
            findings.Add(Create(
    ...
    private static bool IsNonNavigationDivergence(
        ...
        OperationalSourceAvailability sourceAvailability)
    {
        ...
        var isReadableGeneratedNavigationChange =
            observation.ReadState == ManagedTargetReadState.Available
        ...
        return target.State != OperationalTargetState.Current
            && !isReadableGeneratedNavigationChange;
    }
