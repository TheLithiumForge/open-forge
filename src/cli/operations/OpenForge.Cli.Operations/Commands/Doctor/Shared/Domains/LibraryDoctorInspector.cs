...
internal static class LibraryDoctorInspector
{
    ...
    private static void InspectRecord(LibraryRegistrationRead record, ICollection<DoctorFinding> findings)
    {
        if (record.OwnershipObservation is { } cause)
        {
            findings.Add(DoctorDomainSupport.Create(
                DoctorDomainSupport.Information(DoctorFindingKind.LibraryOwnershipObservation, cause),
                PlainSubject(DoctorSubjectKind.Library, ".agents/open-forge.lock.json"),
                DoctorDomainSupport.Provenance(DoctorDomainKind.WorkspaceEntry, DoctorProvenanceSource.LifecycleOwnership,
    ...
    private static void InspectOwnership(LibraryDoctorView libraries, ICollection<DoctorFinding> findings)
    {
        ...
        foreach (var claim in ownership.Document.ManagedPaths())
        {
            ...
            Add(findings,
                claim.Manager == OwnedPathManager.Extension
        ...
        LibraryResidualEvidence? residual = null)
    {
        ...
        return residual is not null
            ...
            : record is not null
                ...
                : PlainSubject(kind, ".agents/open-forge.lock.json");
    }
        ...
        object fact)
    {
        if (record is null)
        {
            return PlainSubject(kind, ".agents/open-forge.lock.json");
        }

        var library = fact switch
        {
            LibraryRegistration registration => new DoctorLibrarySubject(kind, record.Id, registration, null, null, null, null),
    ...
    private static void Add(
        ...
        => findings.Add(DoctorDomainSupport.Create(
            resolution == DoctorResolutionLane.Informational
                ? kind == DoctorFindingKind.LibraryInventoryIncomplete
