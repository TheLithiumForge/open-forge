...
internal sealed record DoctorLibrarySubject
{
    ...
    internal LibraryRegistration? Registration { get; }
    ...
    internal string LibraryIdValue => LibraryId.Value;

    internal string? ProjectionExpectedTarget => Projection?.ExpectedLink.RawRelativeTarget;

    internal string? ProjectionActualTarget => Projection?.Leaf.RelativeFileLink?.RawRelativeTarget;
}
