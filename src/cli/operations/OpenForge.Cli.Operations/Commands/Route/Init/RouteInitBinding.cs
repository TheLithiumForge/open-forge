...
internal static class RouteInitBinding
{
    ...
    internal static CliRequestBinding<RouteInitRequest, RouteInitResult> CreateRequestBinding(
        ...
        RouteInitBindingComponents components)
    {
        ...
        return new CliRequestBinding<RouteInitRequest, RouteInitResult>
        {
            Command = symbols.InitCommand,
        ...
        IReadOnlyList<string> tags)
    {
        ...
        if (tags.FirstOrDefault(tag => !SourceOpenForgeMetadataParser.IsValidTag(tag)) is { } invalidTag)
        {
            return $"--tag {invalidTag} is not a valid tag: use letters and digits with single hyphens.";
        }

        if (tags.GroupBy(tag => tag, StringComparer.Ordinal).FirstOrDefault(group => group.Count() > 1) is { } repeated)
        {
