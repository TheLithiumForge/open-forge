...
internal sealed class RouteListTopologySelectionFindings
{
    ...
    {
        var source = RouteListTopologyProjectionPolicy.ReadSource(_input, node);
        var existingMetadataFinding = _state.Findings.FirstOrDefault(finding =>
        ...
        if (existingMetadataFinding is not null)
        {
            if (existingMetadataFinding.Status != CliSemanticStatus.Attention)
            {
        ...
        _state.AddFinding(
            new RouteListFinding(
                code,
                code is RouteListFindingCode.MetadataMissing or RouteListFindingCode.MetadataMalformed
                    ? CliSemanticStatus.Attention
