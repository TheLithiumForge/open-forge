    ...
    FileExpectationValidator expectationValidator)
{
    ...
    private static bool IsAuthoredLink(
        ...
        MarkdownLinkFact link)
    {
        if (document.Facts.GeneratedRegion.EntriesBlock?.Span is not { } generated
        ...
        SourceLinkDestinationFacts facts)
    {
        ...
        if (facts.Target.Resolution == SourceLinkTargetResolution.FragmentMissing)
        {
            var collision = ReadCurrentTargetCollision(input, facts);
    ...
    private static SourceLinkDestinationFinding? ReadCurrentTargetCollision(
        RouteMoveReferenceScanInput input,
        SourceLinkDestinationFacts facts)
    {
        if (facts.Target.Kind != SourceLinkTargetKind.Local
        ...
        var sourceCatalogue = input.Request.Destination.Inventory.Subject.Catalogue;
        var currentTarget = sourceCatalogue.FindByPath(targetPath);
        if (currentTarget is null)
        {
            return null;
        }

        var candidates = sourceCatalogue.FindAllById(currentTarget.Identity.AutomaticId);
        return candidates.Count > 1
            ? SourceLinkTargetIdentityReader.ReadCollision(
                facts.Target,
                candidates,
                facts.Fragment).Finding
            : null;
    }
