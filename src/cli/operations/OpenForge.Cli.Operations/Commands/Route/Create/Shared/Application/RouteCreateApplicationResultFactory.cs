...
internal static class RouteCreateApplicationResultFactory
{
        ...
        RouteCreateApplicationProgress progress)
    {
        ...
        {
            var directoryReceipt = progress.DirectoryReceipts.FirstOrDefault(candidate =>
                PathEquals(plan, candidate.Creation.LogicalPath, effect.Path));
            if (directoryReceipt is not null)
            {
                var outcome = ReadOutcome(directoryReceipt);
                return effect with
                {
                    Outcome = outcome,
                    Residual = ReadResidual(directoryReceipt, outcome, progress.Verification),
                };
            }

            var directoryAttempted = progress.UncertainDirectoryAttempt is { } uncertainDirectory
                && PathEquals(plan, uncertainDirectory.LogicalPath, effect.Path);
            return effect with
            {
                Outcome = directoryAttempted
                    ? RouteCreateEffectOutcome.CompletionUnknown
                    : RouteCreateEffectOutcome.NotStarted,
                Residual = directoryAttempted
    ...
    private static RouteCreateEffectOutcome ReadOutcome(DirectoryCreationReceipt receipt)
        => (receipt.EffectState, receipt.VerificationState) switch
        {
            (FilesystemEffectState.Applied, FilesystemVerificationState.Verified) =>
                RouteCreateEffectOutcome.Verified,
            ...
            (FilesystemEffectState.NotStarted, FilesystemVerificationState.NotStarted) =>
                RouteCreateEffectOutcome.NotStarted,
            _ => throw new InvalidOperationException(
                "The Route Create directory receipt is incoherent."),
        };
