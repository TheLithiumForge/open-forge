...
internal static class RouteCreateDefinitions
{
    ...
    internal static CliSemanticStatus ReadStatus(RouteCreateFindingCode code)
        => code switch
        {
            ...
            RouteCreateFindingCode.RecoveryArtifactRetained
                or RouteCreateFindingCode.OptionalMetadata => CliSemanticStatus.Attention,
        ...
        IReadOnlyList<RouteCreateFinding> findings)
    {
        ...
        if (status == CliSemanticStatus.Attention
            && Contains(findings, RouteCreateFindingCode.OptionalMetadata))
        {
            return new CliNextAction(
                CommandLines.RouteUpdate,
                "Add an optional description or tag with Route Update when useful.");
        }
