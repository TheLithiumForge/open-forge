...
internal sealed class RouteInspectIdentity
{
    ...
    private static IReadOnlyList<string> MaterializeTags(IEnumerable<string>? tags)
    {
        if (tags is null)
        {
            return [];
        ...
        var materialized = tags.ToArray();
        if (materialized.Any(tag => tag is null))
        {
            throw new ArgumentException("Route-inspect identity tags cannot contain null.", nameof(tags));
        }
