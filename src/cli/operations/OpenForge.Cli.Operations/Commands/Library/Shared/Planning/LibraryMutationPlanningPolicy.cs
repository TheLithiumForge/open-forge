...
internal static class LibraryMutationPlanningPolicy
{
        ...
        out OwnedPath? conflict)
    {
        ...
        conflict = ownership.Document.ManagedPaths().FirstOrDefault(claim => paths.Contains(PortableWorkspacePath.CreatePortableKey(claim.Path)));
        ...
        IEnumerable<string> destinationPaths)
    {
        ...
        foreach (var destinationPath in destinationPaths)
        {
            ...
            for (var length = 1; length < segments.Length; length++)
            {
                ...
                if (ancestor != WorkspaceSettingsDefinitions.DirectoryName)
                {
    ...
    internal static PlannedFileChange? CreateOwnershipChange(
        WorkspaceOwnershipRead ownership,
        ImmutableArray<LibraryOwnership> intended)
        => new WorkspaceOwnershipStore().PlanLibraryOwnership(ownership, intended).Change;
