        ...
        CancellationToken cancellationToken)
    {
        ...
        if (request.OwnershipChange is { } ownershipChange)
        {
            var before = request.Ownership?.Snapshot
                ?? throw new InvalidOperationException("A Library ownership effect requires its exact prior snapshot.");
            targets.Add(ownershipChange.Kind == PlannedFileChangeKind.Create
                ? RecoveryBundleTarget.CreateReversible(ownershipChange, before)
                : RecoveryBundleTarget.Create(ownershipChange, before));
        }
