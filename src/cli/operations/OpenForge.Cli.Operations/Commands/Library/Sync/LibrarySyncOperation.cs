...
internal sealed class LibrarySyncOperation
{
    ...
    private readonly CliPlanConfirmation<LibrarySyncResult, LibrarySyncPlan> _confirmation;

    internal LibrarySyncOperation(
        LibraryPermissionOperation permissions,
        CliPlanConfirmation<LibrarySyncResult, LibrarySyncPlan>? confirmation = null)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var preview = LibrarySyncCompletion.Complete(
            ...
            if (confirmation.State == CliPromptState.Unavailable)
            {
                plan = plan with
                {
                    Findings = plan.Findings.Add(new LibrarySyncFinding
                    {
                        Code = LibrarySyncFindingCode.ConfirmationRequired,
                        Status = CliSemanticStatus.Invalid,
                        LibraryId = request.LibraryId.Value,
                        Path = null,
                        Cause = "Library sync needs confirmation, and this session cannot ask.",
                    }),
                };
                return Complete(request, plan, observations, LibraryMutationOperationSupport.Empty());
            }
        ...
        if (lockResult.State != WorkspaceLockState.Acquired || lockResult.Lease is not { } lease)
        {
            ...
            if (lockResult.State != WorkspaceLockState.Cancelled)
            {
                plan = plan with
                {
                    ...
                    Findings = plan.Findings.Add(new LibrarySyncFinding
                    {
                        Code = LibrarySyncFindingCode.LockUnavailable,
