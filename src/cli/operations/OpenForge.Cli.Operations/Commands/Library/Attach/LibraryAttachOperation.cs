...
internal sealed class LibraryAttachOperation
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var preview = LibraryAttachCompletion.Complete(
            ...
            if (confirmation.State == CliPromptState.Unavailable)
            {
                plan = plan with
                {
                    ...
                    {
                        Code = LibraryAttachFindingCode.ConfirmationRequired,
                        Status = CliSemanticStatus.Invalid,
                        LibraryId = request.LibraryId.Value,
                        Path = null,
                        Cause = "Library attach needs confirmation, and this session cannot ask.",
                    }),
                };
                return Complete(request, plan, observations, LibraryMutationOperationSupport.Empty());
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var mappings = LibraryMutationOperationSupport.ObserveMappings(
                ...
                cancellationToken);
        var navigation = record.State is (LibraryRegistrationReadState.Missing or LibraryRegistrationReadState.Complete)
        ...
        if (lockResult.State != WorkspaceLockState.Acquired || lockResult.Lease is not { } lease)
        {
            ...
            if (lockResult.State != WorkspaceLockState.Cancelled)
            {
                plan = plan with
                {
                    ...
                    Findings = plan.Findings.Add(new LibraryAttachFinding
                    {
                        Code = LibraryAttachFindingCode.LockUnavailable,
    ...
    private static ValueTask<CliPromptReply<bool>> MissingConfirmation(
        LibraryAttachResult _,
        LibraryAttachPlan __,
