...
internal sealed class LibraryInspectOperation
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var read = await LibraryRegistrationReader.ReadAsync(
                _resolver,
                request.Workspace,
                cancellationToken).ConfigureAwait(false);
            if (read.State != LibraryRegistrationReadState.Complete)
            {
        ...
        CancellationToken cancellationToken)
    {
        ...
        return new LibraryInspectResult
        {
            ...
            Result = new LibraryInspectPayload
            {
                ...
                {
                    Path = OpenForge.Cli.Core.Framework.Ownership.WorkspaceOwnershipDefinitions.RelativePath,
                ...
                Source = new LibraryInspectSourceView
                {
                    ...
                    ExcludedCount = inventoryRead.ExcludedPaths.Length,
                },
    ...
    private static LibraryRecordViewState OwnershipRecordState(LibraryRegistrationReadState state)
        => state switch
        {
            LibraryRegistrationReadState.Missing => LibraryRecordViewState.Missing,
            LibraryRegistrationReadState.Malformed => LibraryRecordViewState.Invalid,
            LibraryRegistrationReadState.Unavailable => LibraryRecordViewState.Unavailable,
    ...
    {
        if (read.State != LibraryRegistrationReadState.Malformed
            && read.OwnershipObservation is { } cause)
        {
            return Event(request, CliSemanticStatus.Complete, OwnershipRecordState(read.State),
                LibraryInventoryViewState.NotStarted, LibraryCoverage.Complete,
                LibraryInspectFindingCode.OwnershipObservation, cause);
        }

        if (read.State == LibraryRegistrationReadState.Missing)
        {
