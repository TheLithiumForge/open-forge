...
internal static class CleanupPlanner
{
    internal static CleanupPlan Create(CleanupRequest request, RecoveryBundleCatalogueResult source)
    {
        ...
        var safe = catalogue.Coverage == CleanupCatalogueCoverage.Complete
            && entries.All(CleanupPlan.IsSafeEntry);
        var deletionEntries = safe
            ? entries.Where(CleanupPlan.IsDeletionEntry).ToImmutableArray()
            : [];
        return CleanupPlan.Create(
            ...
            entries,
            deletionEntries,
            safe ? CleanupPlanSafety.Safe : CleanupPlanSafety.Blocked);
    }
