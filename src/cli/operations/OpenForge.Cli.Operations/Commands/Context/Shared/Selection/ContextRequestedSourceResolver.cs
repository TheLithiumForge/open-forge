...
internal sealed class ContextRequestedSourceResolver
{
        ...
        SourceCatalogue catalogue)
    {
        ...
        return Finding(
            ...
            invalidSourceKind: InvalidSourceKind(request, code));
    }
    ...
    private static ContextInvalidSourceKind? InvalidSourceKind(
        ContextResolvedRequest request,
        ContextFindingCode code)
        => code != ContextFindingCode.InvalidSource
            ? null
            : request.Resolution.Form switch
            {
                SourceReferenceKind.SourceId
                    when request.Resolution.State == SourceReferenceResolutionState.Unknown
                    => ContextInvalidSourceKind.UnknownId,
                _ => ContextInvalidSourceKind.Other,
            };
