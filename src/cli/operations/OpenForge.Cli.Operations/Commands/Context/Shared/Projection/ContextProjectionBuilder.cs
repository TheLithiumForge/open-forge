...
internal sealed class ContextProjectionBuilder
{
        ...
        ProjectionAccumulator state)
    {
        ...
        {
            // An entry document is not a routed source, so it carries no route metadata by design.
            if (!source.IsEntryDocument && layer.Kind != SourceLayerKind.Overwrite)
            {
    ...
    private static ContextSourceMetadata Metadata(
        OpenForge.Cli.Core.Framework.Sources.Models.Metadata.SourceAuthoredMetadataFacts metadata)
        => metadata.State switch
        {
            OpenForge.Cli.Core.Framework.Sources.Models.Metadata.SourceAuthoredMetadataState.NotApplicable
                => ContextSourceMetadata.WithoutValues(ContextMetadataState.NotApplicable),
            OpenForge.Cli.Core.Framework.Sources.Models.Metadata.SourceAuthoredMetadataState.Complete
                => ContextSourceMetadata.Complete(
                    metadata.Description ?? throw new InvalidOperationException("Complete source metadata requires a description."),
                    metadata.Tags),
            OpenForge.Cli.Core.Framework.Sources.Models.Metadata.SourceAuthoredMetadataState.Missing
                => ContextSourceMetadata.WithoutValues(ContextMetadataState.Missing),
            OpenForge.Cli.Core.Framework.Sources.Models.Metadata.SourceAuthoredMetadataState.Malformed
                => ContextSourceMetadata.WithoutValues(ContextMetadataState.Malformed),
            _ => throw new ArgumentOutOfRangeException(nameof(metadata), metadata.State, "The source metadata state is not defined."),
        };
