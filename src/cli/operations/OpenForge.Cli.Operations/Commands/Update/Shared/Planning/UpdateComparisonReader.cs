...
internal sealed class UpdateComparisonReader(PhysicalPathResolver physicalPathResolver)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var componentPath = workspace.LexicalRoot;
        foreach (var component in relativePath.Split('/'))
        {
            componentPath = Path.Combine(componentPath, component);
            try
            {
                if ((File.GetAttributes(componentPath) & FileAttributes.ReparsePoint) != 0)
                    return Blocked(relativePath, "The Update destination crosses a link or reparse point.");
            }
