...
internal sealed class ExtensionUpdateReconciler(FileExpectationValidator validator)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var current = input.Ownership.Extensions;
        var topology = input.Topology;
        var packageById = packages.ToDictionary(package => package.Id, StringComparer.Ordinal);
        var currentPackages = input.Ownership.Extensions.ToDictionary(package => package.Id, StringComparer.Ordinal);
        ...
        var intendedPackages = input.Ownership.Extensions.Select(installedPackage =>
        {
            ...
            var paths = request.Prune
                ...
                : installedPackage.Paths.Concat(newPathsByPackage[package.Id])
                    .ToHashSet(StringComparer.Ordinal);
            return new ExtensionOwnership(
                package.Id, package.Version, input.SourceIdentity,
                [.. package.Dependencies.Order(StringComparer.Ordinal)],
                [.. paths.Order(StringComparer.Ordinal)],
                installedPackage.Regions);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var currentFingerprint = snapshot.Kind != FileExpectationKind.File ? null
            : ExtensionDestinationPolicy.IsImplicit(input.Path)
                ? Fingerprint(snapshot.Bytes.ToArray()).Fingerprint
                : FileExpectation.Hash(snapshot.Bytes.AsSpan());
        (string Fingerprint, MarkdownFingerprintState Kind) intendedIdentity = intendedBytes is null ? default
            : ExtensionDestinationPolicy.IsImplicit(input.Path)
                ? Fingerprint(intendedBytes)
                : (FileExpectation.Hash(intendedBytes), MarkdownFingerprintState.ExactBytes);
        var same = intendedBytes is not null && snapshot.Kind == FileExpectationKind.File
            && string.Equals(currentFingerprint, intendedIdentity.Fingerprint, StringComparison.Ordinal);
        var currentState = snapshot.Kind == FileExpectationKind.Missing
            ? ExtensionUpdateComparisonCurrentState.Missing
            : same ? ExtensionUpdateComparisonCurrentState.Same
            : ExtensionUpdateComparisonCurrentState.Changed;
        var intendedState = input.SourceFile is null ? ExtensionUpdateComparisonIntendedState.Retired
            : input.Owners.Count == 0 ? ExtensionUpdateComparisonIntendedState.New
            : same ? ExtensionUpdateComparisonIntendedState.Same
            : ExtensionUpdateComparisonIntendedState.Changed;
        var retirement = input.SourceFile is not null ? ExtensionUpdateRetirementEligibility.NotApplicable
            : snapshot.Kind == FileExpectationKind.File && input.Owners.Count > 0
        ...
        comparisons.Add(new ExtensionUpdateComparison(
            ...
            ReadFingerprintKind(intendedIdentity.Kind),
            currentFingerprint,
        ...
        if (input.Owners.Count == 0 && snapshot.Kind == FileExpectationKind.File)
        {
            return Stop(
                stage,
                new ExtensionUpdateFinding(
        ...
        var retainedOwner = input.Owners.Any(owner => !selectedIds.Contains(owner));
            ...
            && plannedPaths.Add(input.Path))
        {
            ...
            if (snapshot.Kind == FileExpectationKind.Missing)
            {
                logicalAction = input.Owners.Count == 0 ? ExtensionUpdateChangeAction.Create : ExtensionUpdateChangeAction.Restore;
            }
        ...
        ReconciliationStage stage)
    {
        ...
        if (!input.Request.Prune)
        {
            findings.Add(new ExtensionUpdateFinding(
                ...
                "A retired managed file was kept because --prune was not supplied.",
                input.Path));
            return null;
        }

        var remainingOwners = input.Owners
        ...
        if (snapshot.Kind == FileExpectationKind.File && plannedPaths.Add(input.Path))
        {
            effects.Add(Effect(new PackageEffectInput
            {
                Path = input.Path,
    ...
    private (string Fingerprint, MarkdownFingerprintState Kind) Fingerprint(byte[] bytes)
    {
        ...
        {
            return (FileExpectation.Hash(bytes), MarkdownFingerprintState.ExactBytes);
        }
        return (facts.Sha256, facts.State == MarkdownFingerprintState.Semantic
            ? MarkdownFingerprintState.Semantic : MarkdownFingerprintState.ExactBytes);
    }
    ...
    private static ExtensionUpdateComparisonFingerprintKind ReadFingerprintKind(MarkdownFingerprintState kind)
        => kind == MarkdownFingerprintState.Semantic
    ...
    private sealed class ReconciliationStage
    {
        ...
        internal required ImmutableArray<ExtensionOwnership> Current { get; init; }
    }
}
