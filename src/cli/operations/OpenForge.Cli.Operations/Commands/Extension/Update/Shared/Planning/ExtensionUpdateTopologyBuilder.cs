...
internal sealed class ExtensionUpdateTopologyBuilder
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var omittedRegions = _independentMetadataProof.ReadOmittableRegions(
            projection,
            formation,
            regionInputs,
            metadata,
        ...
        if (projection.Regions.Any(region => region.State != GeneratedNavigationRegionState.Available
            && !omittedRegions.Contains(region.CanonicalPath)))
        {
    ...
    private static PayloadBytes ReadPayload(ExtensionPackageFileFact file)
    {
        ...
        if (portableKey == PortableWorkspacePath.CreatePortableKey(WorkspaceOwnershipDefinitions.RelativePath)
            || SourceFormClassifier.TryClassify(portableKey, out var form)
            && form == SourceDocumentForm.OverwriteCompanion)
        {
            throw new InvalidDataException(
                "An Extension payload targets a reserved workspace state or overwrite-companion path.");
        }
