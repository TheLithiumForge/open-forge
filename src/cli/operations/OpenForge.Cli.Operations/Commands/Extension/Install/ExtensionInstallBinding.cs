...
internal static class ExtensionInstallBinding
{
    internal static ExtensionInstallSymbols CreateSymbols(Command extensionGroup)
    {
        ...
        return new ExtensionInstallSymbols(command, ids, source, all, force, automatic, dryRun, allowPath);
    }

    internal static CliRequestBinding<ExtensionInstallRequest, ExtensionInstallResult> CreateRequestBinding(
        ExtensionInstallSymbols symbols,
        CliHelpContent help,
        ExtensionInstallOperation operation)
    {
        var binder = new ExtensionInstallRequestBinder(symbols);
        return new CliRequestBinding<ExtensionInstallRequest, ExtensionInstallResult>
        {
...
internal sealed class ExtensionInstallRequestBinder(ExtensionInstallSymbols symbols)
{
        ...
        CliPresentation presentation)
    {
        ...
        return new ExtensionInstallRequest(
            ...
            input.Force,
            input.Automatic,
            allowInteraction: presentation.Format == CliFormat.Text
                && !input.Automatic,
            allowPath: [.. CliOptionResultFactsReader.ReadValues(result, _symbols.AllowPath)]);
    }
