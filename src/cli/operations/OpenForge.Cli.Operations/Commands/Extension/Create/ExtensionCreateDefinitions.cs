...
internal static class ExtensionCreateDefinitions
{
    ...
    internal static readonly CliNextAction MissingInputNext = new(
        "open-forge extension create <id> --path <folder>",
        "Supply the Extension ID and package folder, then rerun the request.");

    internal static readonly CliNextAction IncompleteNext = new(
        "open-forge extension create --detail debug",
    ...
    internal static readonly CliNextAction InterruptedNext = new(
        CommandLines.ExtensionCreate,
        "Inspect the retained destination state, then rerun Extension Create if appropriate.");

    internal static readonly CliNextAction ConfirmationRequiredNext = new(
        "open-forge extension create --automatic",
        "Rerun extension create with --automatic or use an interactive terminal to supply confirmation.");
    ...
    internal static string ReadFindingCode(ExtensionCreateFindingCode code)
        => code switch
        {
            ...
            ExtensionCreateFindingCode.ConfirmationRequired => "extension-create.confirmation-required",
    ...
    internal static CliNextAction? ReadNext(
        ...
        ExtensionCreateFindingCode? findingCode)
        => findingCode == ExtensionCreateFindingCode.ConfirmationRequired
            ? ConfirmationRequiredNext
            : ReadNext(status);
    ...
    internal static CliNextAction? ReadNext(
        ...
        => finding switch
        {
            { Code: ExtensionCreateFindingCode.ConfirmationRequired } => ConfirmationRequiredNext,
            {
                Code: ExtensionCreateFindingCode.InvalidInput,
                Cause: "A stable ID is required." or "A catalogue path is required.",
            } => MissingInputNext,
            _ => ReadNext(status),
        };
}
