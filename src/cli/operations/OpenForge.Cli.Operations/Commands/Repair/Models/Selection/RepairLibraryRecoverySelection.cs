...
internal sealed record RepairLibraryRecoveryProposal
{
    ...
    internal string LibraryIdValue => Evidence.LibraryId.Value;

    internal string TargetPath => Evidence.Entry.Input.Context.Entry.TargetPath;
}
...
internal sealed record RepairSelectedLibraryRecovery
{
    internal RepairSelectedLibraryRecovery(RepairLibraryRecoveryProposal proposal, ImmutableArray<RepairSelectionOrigin> origins)
    {
        ...
        if (origins.IsDefaultOrEmpty || origins.Any(origin => origin is not RepairSelectionOrigin.Automatic and not RepairSelectionOrigin.Prompt))
        {
