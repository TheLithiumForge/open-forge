...
internal static class RepairDefinitions
{
    ...
    internal const string CancelledMessage = "Repair was cancelled. Nothing was changed.";

    internal const string ConfirmationRequiredMessage = "Repair needs confirmation, and this session cannot ask.";

    internal const string SelectionRequiredMessage = "Repair needs to know which repairs to apply, and this session cannot ask.";
    ...
    internal static string ReadMachineName(RepairSelectionMode mode)
        => mode switch
        {
            RepairSelectionMode.InteractivePrompt => "interactive-prompt",
    ...
    internal static string ReadMachineName(RepairSelectionOrigin origin)
        => origin switch
        {
            ...
            RepairSelectionOrigin.Prompt => "prompt",
    ...
    internal static string ReadMachineName(RepairDependencyDomain domain)
        => domain switch
        {
            ...
            RepairDependencyDomain.LibraryRegistration => "library-record",
    ...
    internal static string ReadMachineName(RepairConflictKind kind)
        => kind switch
        {
            ...
            RepairConflictKind.SelectionUnmatched => "selection-unmatched",
            RepairConflictKind.IncompleteFacts => "incomplete-facts",
            _ => Undefined(nameof(kind), kind),
        };

    internal static RepairFindingCode ReadConflictFindingCode(RepairConflictKind kind)
        => kind switch
        {
            RepairConflictKind.OverlappingChanges => RepairFindingCode.PlanConflict,
            RepairConflictKind.ExpectedStateMismatch => RepairFindingCode.TargetChanged,
            RepairConflictKind.TargetIdentityMismatch => RepairFindingCode.FactsConflicting,
            RepairConflictKind.UnsafeBoundary => RepairFindingCode.TargetUnsafe,
            RepairConflictKind.SelectionUnmatched => RepairFindingCode.ProposalUnavailable,
            RepairConflictKind.IncompleteFacts => RepairFindingCode.DiagnosisIncomplete,
    ...
    internal static RepairFindingDefinition Read(RepairFindingCode code)
        => code switch
        {
            ...
            RepairFindingCode.ConfirmationRequired => Definition(
                code,
            ...
            RepairFindingCode.ProposalUnavailable => Definition(
                ...
                CliSemanticStatus.Incomplete),
            RepairFindingCode.FactsConflicting => Definition(
                code,
                "repair.facts-conflicting",
                CliSemanticStatus.Blocked),
        ...
        bool recoveryArtifactRetained = false)
    {
            ...
            && findings.Any(finding => finding.Code == RepairFindingCode.SelectionRequired))
        {
            return new CliNextAction(
                ...
                "Select safe-exact repairs with --automatic, provide an explicit --relink target, or use the interactive Repair prompt.");
        }

        if (status == CliSemanticStatus.Invalid
            && findings.Any(finding => finding.Code == RepairFindingCode.ConfirmationRequired))
        {
            return new CliNextAction(
                "open-forge repair --automatic",
                "Run Repair with --automatic or provide explicit --relink authority when confirmation is unavailable.");
        }
        ...
        if (findings.Any(finding => finding.Code == RepairFindingCode.ProposalUnavailable))
        {
            return new CliNextAction(
        ...
        if (findings.Any(finding => finding.Code == RepairFindingCode.FactsConflicting))
        {
            return new CliNextAction(
                "open-forge doctor --detail standard",
                "Inspect the conflicting Repair facts before relying on this result.");
        }
        ...
        return status switch
        {
            ...
            CliSemanticStatus.Attention => new CliNextAction(
                CommandLines.Repair,
                "Review remaining guided or manual findings in Repair, then select a candidate in the interactive prompt or provide an explicit --relink target."),
            CliSemanticStatus.Failed => new CliNextAction(
                "open-forge repair --detail debug",
