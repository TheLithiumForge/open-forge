...
internal sealed record InstallPlan
{
    ...
    public required FrameworkLifecycleState IntendedLifecycle { get; init; }
    ...
    public required InstallFileEffect? LifecycleEffect { get; init; }
    ...
    public IReadOnlyList<PlannedFileChange> FileChanges =>
        LifecycleEffect is null
            ? TargetEffects.Select(effect => effect.Change).ToArray()
            : TargetEffects.Select(effect => effect.Change)
                .Append(LifecycleEffect.Change)
                .ToArray();

    public IReadOnlyList<RecoveryBundleTarget> RecoveryTargets =>
        LifecycleEffect is null
            ? TargetEffects.Select(effect => effect.RecoveryTarget).ToArray()
            : TargetEffects.Select(effect => effect.RecoveryTarget)
                .Append(LifecycleEffect.RecoveryTarget)
                .ToArray();
...
internal enum InstallPreservationVerificationState
{
    Verified,
    Changed,
    Incomplete,
...
internal sealed record InstallPreservationVerification
{
    public required InstallPreservationVerificationState State { get; init; }
