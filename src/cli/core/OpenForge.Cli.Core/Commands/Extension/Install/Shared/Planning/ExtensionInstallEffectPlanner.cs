    ...
    FileExpectationValidator validator)
{
        ...
        CancellationToken cancellationToken)
    {
        var lifecyclePlan = _lifecycleStore.PlanExtensionUpdate(
            input.LifecycleRead,
            input.TargetState.IntendedLifecycle);
        ...
        {
            if (input.LifecycleRead.File is not { } lifecycleBefore)
            {
                return Stop(
                    ExtensionInstallFindingCode.LifecycleUnavailable,
                    "Lifecycle publication requires an exact prior lifecycle snapshot.");
            }
        ...
        return new ExtensionInstallEffectPlan(
            ...
            lifecyclePlan.State == LifecycleWritePlanState.Unchanged
                ? ExtensionInstallLifecycleAction.Preserve
                : ExtensionInstallLifecycleAction.Publish,
            finding: null);
    }
