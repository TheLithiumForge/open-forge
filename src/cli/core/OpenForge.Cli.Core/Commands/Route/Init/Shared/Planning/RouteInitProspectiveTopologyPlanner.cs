...
internal sealed record RouteInitLifecycleEffectInput(
...
internal sealed class RouteInitProspectiveTopologyPlanner
{
    ...
    internal RouteInitProspectiveEffectPlanBuild AssembleEffects(
        ...
        RouteInitLifecycleEffectInput? lifecycleEffect)
    {
        ArgumentNullException.ThrowIfNull(topology);
        ...
        try
        {
            ...
            if (lifecycleEffect is not null)
            {
                var boundary = AddLifecycleEffect(lifecycleEffect, changes, recovery);
    ...
    private static string? AddLifecycleEffect(
        RouteInitLifecycleEffectInput lifecycleEffect,
        ICollection<PlannedFileChange> changes,
        ICollection<RecoveryBundleTarget> recovery)
    {
        return lifecycleEffect.WritePlan.State switch
        {
            LifecycleWritePlanState.Unchanged => null,
            LifecycleWritePlanState.Blocked => lifecycleEffect.WritePlan.Cause
                ?? "The Route Init lifecycle effect is blocked.",
            LifecycleWritePlanState.Planned => AddPlannedLifecycleEffect(
                lifecycleEffect,
                changes,
                recovery),
            _ => throw new ArgumentOutOfRangeException(
                nameof(lifecycleEffect),
                lifecycleEffect.WritePlan.State,
                "The lifecycle write-plan state is not defined."),
        };
    }

    private static string? AddPlannedLifecycleEffect(
        RouteInitLifecycleEffectInput lifecycleEffect,
        ICollection<PlannedFileChange> changes,
        ICollection<RecoveryBundleTarget> recovery)
    {
        var change = lifecycleEffect.WritePlan.Change
            ?? throw new ArgumentException(
                "A planned Route Init lifecycle effect requires one file change.",
                nameof(lifecycleEffect));
        if (change.Kind is not (PlannedFileChangeKind.Replace
            or PlannedFileChangeKind.ReplaceGeneratedRegion
            or PlannedFileChangeKind.Delete)
            || change.Expectation != lifecycleEffect.Before.Expectation
            || !lifecycleEffect.Before.HasBytes)
        {
            return "A Route Init lifecycle effect requires one exact existing-target recovery snapshot.";
        }

        changes.Add(change);
        recovery.Add(RecoveryBundleTarget.Create(change, lifecycleEffect.Before));
        return null;
    }
