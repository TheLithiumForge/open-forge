...
internal sealed partial class RouteMovePostMoveObserver
{
    ...
    private static string? ReadLifecycleMismatch(
        LifecycleOwnershipReadResult expected,
        LifecycleOwnershipReadResult actual)
    {
        if (expected.Framework != actual.Framework || expected.Extensions != actual.Extensions)
        {
            return "The final Route Move lifecycle trust observation changed.";
        }

        if (expected.LifecycleFileExpectation != actual.LifecycleFileExpectation)
        {
            return "The final Route Move lifecycle file expectation changed.";
        }

        return expected.Claims.SequenceEqual(actual.Claims)
            ? null
            : "The final Route Move lifecycle claims changed.";
    }
