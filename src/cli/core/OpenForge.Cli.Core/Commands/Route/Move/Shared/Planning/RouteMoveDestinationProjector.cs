...
internal static class RouteMoveDestinationProjector
{
        ...
        RouteMoveCategoryInventory inventory)
    {
        ...
        var destinationPath = operation.DestinationTarget;
        if (!SourceLogicalPath.IsCanonicalSource(destinationPath))
        {
            return Stop(
                inventory,
                ...
                "The Route Move destination must be one exact canonical .agents path.");
        }
