...
internal sealed record LifecycleObservation(
    LifecycleOwnershipReadState Framework,
    string? FrameworkCause,
    LifecycleOwnershipReadState Extensions,
    string? ExtensionsCause,
    FileExpectation? Expectation,
    ImmutableArray<LifecycleOwnershipClaim> Claims)
{
    internal bool Matches(LifecycleObservation actual)
        => Framework == actual.Framework
            && FrameworkCause == actual.FrameworkCause
            && Extensions == actual.Extensions
            && ExtensionsCause == actual.ExtensionsCause
