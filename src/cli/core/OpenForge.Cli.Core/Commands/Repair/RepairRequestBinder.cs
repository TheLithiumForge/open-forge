...
internal sealed class RepairRequestBinder(RepairSymbols symbols)
{
        ...
        CliInvocation invocation)
    {
        ...
        var allowInteraction = invocation.Presentation.Format == CliOutputFormat.Human
            && !input.Automatic
            && relinks.Count == 0;
        try
        {
    ...
    internal RepairBindingInput ReadInput(ParseResult result)
    {
        ...
        return new RepairBindingInput(
            ...
            ReadRelinkValues(result, _symbols.Relink));
    }

    private static string[] ReadRelinkValues(ParseResult result, Option<string[]> option)
    {
        try
        {
        ...
        [NotNullWhen(false)] out RepairFinding? invalid)
    {
        ...
        if (RepairRelinkNormalizer.HasContradictions(distinct))
        {
            ...
            invalid = new RepairFinding(
                RepairFindingCode.ContradictoryRelink,
                "Contradictory --relink tuples cannot address one source occurrence.");
            return false;
        }
