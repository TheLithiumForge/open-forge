...
internal static class RepairSelectionPlanner
{
    internal static RepairPlanningSelection Resolve(
        ...
        IReadOnlyList<RepairRelinkRequest>? wizardRelinks = null)
    {
        ...
        var relinks = wizardRelinks ?? request.Relinks;
        foreach (var entry in entries)
        {
            var explicitRelink = relinks.SingleOrDefault(relink =>
                MatchesOccurrence(relink, entry.Input.Proposal));
            var selection = Select(request, entry.Input, explicitRelink, wizardRelinks is not null);
        ...
        bool wizardAnswered)
    {
        ...
        {
            origins.Add(wizardAnswered ? RepairSelectionOrigin.Wizard : RepairSelectionOrigin.ExplicitRelink);
        }
        else if (!wizardAnswered && request.SelectionMode == RepairSelectionMode.InteractiveWizard && proposal.IsSafeExact)
        {
