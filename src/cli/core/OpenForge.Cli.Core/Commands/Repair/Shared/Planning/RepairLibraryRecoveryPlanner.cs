...
internal static class RepairLibraryRecoveryPlanner
{
    internal static RepairPlan Build(RepairLibraryPlanningInput input)
    {
        ...
        if (input.References.IsDefault || input.Libraries.IsDefault || input.WizardRelinks.IsDefault
            || input.WizardLibraries is { IsDefault: true })
        {
        ...
        var referencePlan = input.WizardLibraries is null
            ? RepairPlanner.Build(input.Request, input.References)
            : RepairPlanner.Build(input.Request, input.References, input.WizardRelinks);
        ...
        foreach (var value in selectedValues)
        {
            ...
            steps.Add(new RepairLibraryRecoveryStep
            {
                ...
                [
                    ...
                    RepairDependencyDomain.LibraryRecord,
        ...
        List<RepairConflict> conflicts)
    {
        ...
        foreach (var proposal in input.Libraries)
        {
            ...
            if (input.WizardLibraries is { } wizard
                && wizard.Any(value => SameEvidence(value.Evidence, proposal.Evidence)))
            {
                origins.Add(RepairSelectionOrigin.Wizard);
            }
        ...
        if (input.WizardLibraries is { } requested)
        {
            foreach (var proposal in requested)
            {
                ...
                {
                    conflicts.Add(new RepairConflict(
                        RepairConflictKind.MissingAuthority,
