...
internal sealed record UpdateComparison
{
    ...
    private void ValidateSourceProvenance()
    {
        ...
        if (string.IsNullOrWhiteSpace(SourceAssetPath) || SourceAssetPresentInCurrentInventory is null)
        {
    ...
    private void ValidateFingerprints()
    {
        ...
        ValidateFingerprint(BaselineFingerprint, nameof(BaselineFingerprint));
    ...
    private void ValidateStates()
    {
        ...
        if ((IntendedState == UpdateComparisonIntendedState.New) != (BaselineFingerprint is null))
        {
            throw new ArgumentException(
                "Baseline fingerprint nullability must identify a genuinely new intended target.");
        }
    }
