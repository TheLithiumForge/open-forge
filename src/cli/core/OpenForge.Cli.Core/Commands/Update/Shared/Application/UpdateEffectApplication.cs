...
internal sealed class UpdateEffectApplication(FileChangeApplier fileApplier)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var changes = execution.Effects.Count + (execution.LifecycleChange is null ? 0 : 1);
        ...
        FileChangeReceipt? lifecycleReceipt = null;
        if (execution.LifecycleChange is { } lifecycle)
        {
            lifecycleReceipt = await ApplyAsync(
                    lease,
                    lifecycle,
                    checks[checkIndex],
                    preparation,
                    cancellationToken)
                .ConfigureAwait(false);
            if (!IsVerified(lifecycleReceipt))
            {
                return Failed(
                    receipts,
                    lifecycleReceipt,
                    lifecycleReceipt.EffectState == FilesystemEffectState.Applied
                        ? UpdateFindingCode.LifecyclePublicationFailed
                        : ReadFinding(lifecycleReceipt),
                    lifecycleReceipt.Cause ?? "Update lifecycle publication was not verified.");
            }
        }

        return new UpdateApplicationAttempt(receipts, lifecycleReceipt, Finding: null);
    }
