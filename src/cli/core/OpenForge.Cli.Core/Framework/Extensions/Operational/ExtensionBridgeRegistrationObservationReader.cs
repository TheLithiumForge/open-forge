...
internal sealed class ExtensionBridgeRegistrationObservationReader
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (lifecycle.State == LifecycleReadState.Complete
            && lifecycle.Trust == LifecycleExtensionTrust.Absent
            && lifecycle.Paths.Count == 0)
        {
            return ExtensionBridgeRegistrationFacts.Complete([]);
        }

        if (lifecycle.Trust == LifecycleExtensionTrust.Absent)
        {
            return ExtensionBridgeRegistrationFacts.Incomplete(
        ...
        {
            return ExtensionBridgeRegistrationFacts.Incomplete(
                lifecycle.Cause ?? "Trusted Extension lifecycle facts are unavailable.");
        }

        var candidateRead = ReadCandidates(lifecycle, sources);
    ...
    private static CandidateRead ReadCandidates(
        ...
        IReadOnlyList<ExtensionSourceObservation> sources)
    {
        var packages = lifecycle.Packages.ToDictionary(package => package.Id, StringComparer.Ordinal);
        var candidates = new List<BridgeCandidate>();
        foreach (var path in lifecycle.Paths)
        {
            var ownerCandidates = new List<BridgeCandidate>();
            foreach (var owner in path.Owners.Order(StringComparer.Ordinal))
            {
                if (!packages.TryGetValue(owner, out var installed)
                    || installed.Source is null)
                {
                ...
                if (!SourceFormClassifier.TryClassify(path.Path, out var form)
