...
internal sealed record RelativeFileLinkReceipt
{
    ...
    {
        ArgumentNullException.ThrowIfNull(after);
        if (MatchesIntended(effect, after))
        {
            throw new ArgumentException(
    ...
    internal static RelativeFileLinkReceipt CompletionUnknown(
        ...
        string cause)
    {
        if (after is not null && MatchesIntended(effect, after))
        {
            throw new ArgumentException(
    ...
    private static bool MatchesIntended(
        ...
        => effect.Intended.State == observation.State
            && (effect.Intended.Link is null
                || effect.Intended.Link == observation.RelativeFileLink);
