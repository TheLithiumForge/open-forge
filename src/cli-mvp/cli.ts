// Frozen MVP reference. The planned replacement source belongs under ../open-forge-cli/OpenForge.Cli/.
...
const routeTypeTags = ["Directive", "Guidance", "Pattern", "Skill", "Template", "Workflow", "Map", "Memory"] as const;
const routeTypeTagsByLower = new Map(routeTypeTags.map((tag) => [tag.toLowerCase(), tag]));
const extensionContentKindOrder = ["skill", "workflow", "directive", "guidance", "pattern", "template", "map", "memory", "pack", "other"] as const;
...
async function main(): Promise<void> {
  ...
  try {
    if (command === "install") {
      ...
      assertNoExtraArgs(args, 1, `Usage: ${cliName} help`);
      printHelp();
    } else {
      throw new Error(`Unknown command: ${command}. Run ${cliName} help to list supported commands.`);
    }
  } catch (error) {
...
async function install(installArgs: string[]): Promise<void> {
  ...
  const usage = `Usage: ${cliName} install [target] [--pro]`;
  ...
  await assertCoreMapTopology(targetRoot, preserveMissingDefaults);
  ...
  if (!proMode) {
    await assertPlannedFilesAreGitVisible(targetRoot, [...corePlan.entries.map((entry) => entry.targetFile), ...potentialIndexFiles]);
  }
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `Core installation failed (${error instanceof Error ? error.message : String(error)}) and rollback also failed (${rollbackError instanceof Error ? rollbackError.message : String(rollbackError)})`,
      );
    }
    throw error;
  }
  ...
  console.log(
    `Updated ${corePlan.copied} managed files, reconciled ${corePlan.reconciled} managed route entrypoints through scopes, patched ${corePlan.patched} entry files, rebuilt ${generatedRegions} generated regions.`,
  );
  await printPostInstallCheckpoint(targetRoot, "Core", proMode);
  console.log(`After reviewing and committing Core, inspect optional extensions with ${cliName} extend --list or ${cliName} extend --select ${JSON.stringify(targetRoot)}.`);
}
...
async function extend(extendArgs: string[]): Promise<void> {
  ...
  if (normalizedArgs[0] === "--list") {
    if (dryRun) {
      throw new Error("--dry-run previews changes and cannot be combined with --list");
    }

    assertNoExtraArgs(normalizedArgs, 1, `Usage: ${cliName} extend --list`);
    await listBundledExtensions();
    return;
  }

  if (normalizedArgs[0] === "--remove") {
    const usage = `Usage: ${cliName} extend --remove <id[,id...]> [target] [--dry-run] [--pro]`;
  ...
  if (normalizedArgs[0] === "--select" || normalizedArgs.length === 0) {
    const usage = `Usage: ${cliName} extend --select [target] [--dry-run] [--pro]`;
  ...
  if (idsValue) {
    const usage = `Usage: ${cliName} extend --ids <id[,id...]> [target] [--dry-run] [--pro]`;
  ...
  const usage = `Usage: ${cliName} extend <extension-source-or-id> [target] [--dry-run] [--pro]`;
...
async function indexRoutes(indexArgs: string[]): Promise<void> {
  const usage = `Usage: ${cliName} index [target]`;
...
function readIdsValue(args: string[]): { ids: string[]; consumed: number } | null {
  ...
  if (first === "--ids") {
    const value = args[1];
    if (!value) {
      throw new Error(`Usage: ${cliName} extend --ids <id[,id...]> [target] [--dry-run] [--pro]`);
    }
...
async function installExtensions(extensionArgs: string[], targetArg: string, dryRun = false, proMode = false): Promise<void> {
  ...
  if (!dryRun) {
    if (!proMode && !(await hasCoreInstallation(targetRoot))) {
      throw new Error(
        `Core is not installed at ${targetRoot}. Run ${cliName} install ${JSON.stringify(targetRoot)}, review and commit Core, then install extensions. Use --pro only to intentionally bypass this checkpoint.`,
      );
    }
  ...
  console.log(
    `Created ${counts.create}, updated ${counts.update}, deleted ${counts.delete}, left ${counts.unchanged} extension files unchanged, and rebuilt ${generatedRegions} generated regions.`,
  );
  console.log(`Scope review: ${scopes.routed} routed, ${scopes.baseline} baseline-loading, ${scopes.executable} skill-executable, ${scopes.workspace} outside-.agents files.`);
...
type GitCheckpointState = { kind: "repo"; root: string; pathspec: string | null; changes: string } | { kind: "none" } | { kind: "error"; message: string };
...
async function hasCoreInstallation(targetRoot: string): Promise<boolean> {
  ...
  try {
    await Promise.all([assertExistingPathInside(agentsFile, targetRoot, "Core AGENTS.md"), assertExistingPathInside(loaderFile, targetRoot, "Core loader")]);
  } catch {
    return false;
  }
  const [agentsText, loaderText] = await Promise.all([fs.readFile(agentsFile, "utf8"), fs.readFile(loaderFile, "utf8")]);
  return (
    agentsText.includes("<!-- open-forge:start -->") &&
...
async function hasRecognizableCoreFootprint(targetRoot: string): Promise<boolean> {
  const anchorFiles = [path.join(targetRoot, "AGENTS.md"), path.join(targetRoot, "CLAUDE.md"), path.join(targetRoot, agentsDirectoryName, "loader.md")];
...
async function assertCoreMapTopology(targetRoot: string, hasRecognizableCore: boolean): Promise<void> {
  const agentsRoot = path.join(targetRoot, agentsDirectoryName);
  const agentsStat = await lstatIfExists(agentsRoot);
  let hasLegacyRoot = false;
  let hasMapRoot = false;
  if (agentsStat?.isDirectory() && !agentsStat.isSymbolicLink()) {
    const entries = await fs.readdir(agentsRoot, { withFileTypes: true });
    hasLegacyRoot = entries.some((entry) => portableExtensionPathKey(entry.name) === "workspace");
    hasMapRoot = entries.some((entry) => portableExtensionPathKey(entry.name) === "maps");
  }
  const hasRecognizableMap = hasMapRoot && hasRecognizableCore ? await hasRecognizableMapPrimitive(targetRoot) : false;
  assertCoreMapTopologyState(hasLegacyRoot, hasMapRoot, hasRecognizableCore, hasRecognizableMap);
}

async function hasRecognizableMapPrimitive(targetRoot: string): Promise<boolean> {
  const mapFile = path.join(targetRoot, agentsDirectoryName, "maps", "_maps.md");
  const loaderFile = path.join(targetRoot, agentsDirectoryName, "loader.md");
  const [mapStat, loaderStat] = await Promise.all([lstatIfExists(mapFile), lstatIfExists(loaderFile)]);
  if (!mapStat?.isFile() || mapStat.isSymbolicLink() || !loaderStat?.isFile() || loaderStat.isSymbolicLink()) {
    return false;
  }
  try {
    await Promise.all([assertExistingPathInside(mapFile, targetRoot, "Core Map entrypoint"), assertExistingPathInside(loaderFile, targetRoot, "Core loader")]);
    const [mapText, loaderText] = await Promise.all([fs.readFile(mapFile, "utf8"), fs.readFile(loaderFile, "utf8")]);
    const mapTags = new Set(readMetadata(mapText).tags.map((tag) => tag.toLowerCase()));
    const loaderEntry = readGeneratedEntries(loaderText).find((entry) => portableExtensionPathKey(entry.path) === "maps/_maps.md");
    return mapTags.has("core") && mapTags.has("map") && loaderEntry?.tags.some((tag) => tag.toLowerCase() === "map") === true;
  } catch {
    return false;
  }
}

function assertCoreMapTopologyState(hasLegacyRoot: boolean, hasMapRoot: boolean, hasRecognizableCore: boolean, hasRecognizableMap: boolean): void {
  if (hasLegacyRoot && hasMapRoot) {
    throw new Error(
      "Core installation found both .agents/workspace/ and .agents/maps/. It will not merge or delete either root. Establish a clean Git baseline, resolve Extension ownership, consolidate the content under .agents/maps/, update authored references, run index, review and commit the migration, then run install and doctor.",
    );
  }
  if (hasLegacyRoot) {
    throw new Error(
      "Core installation found the former Workspace primitive at .agents/workspace/. It will not move user content automatically. Establish a clean Git baseline, resolve Extension ownership, move the root to .agents/maps/, rename _workspace.md and its overwrite companion to _maps.md, update authored references, run index, review and commit the migration, then run install and doctor.",
    );
  }
  if (hasMapRoot && !hasRecognizableCore) {
    throw new Error(
      "Core installation found .agents/maps/ without recognizable Core anchors. It will not overwrite an unowned Map root. Review or move that content before installing Open Forge.",
    );
  }
  if (hasMapRoot && !hasRecognizableMap) {
    throw new Error(
      "Core installation found .agents/maps/ without a recognized #Map entrypoint and loader entry. It will not overwrite ambiguous content. Review the route and generated loader entry before reinstalling Open Forge.",
    );
  }
}
...
async function enforceGitCheckpoint(targetRoot: string, action: string, proMode: boolean): Promise<void> {
  if (proMode) {
    console.log(`${action}: --pro bypassed the Git and Core checkpoints. Installation safety checks are still active.`);
    return;
  }

  const state = await inspectGitCheckpoint(targetRoot);
  if (state.kind === "error") {
    throw new Error(`${action} could not check Git state: ${state.message}. Fix Git access or rerun with --pro to bypass the Git checkpoint.`);
  }
  if (state.kind === "none") {
    ...
    throw new Error(
      `${action} requires a Git repository so the changes can be reviewed. Run git init in ${JSON.stringify(targetRoot)} and create a clean baseline, or rerun with --pro to bypass the Git checkpoint.`,
    );
  }
  if (state.changes) {
    const preview = state.changes.split(/\r?\n/).filter(Boolean).slice(0, 8).join(", ");
    throw new Error(
      `${action} requires a clean Git state for ${JSON.stringify(targetRoot)}. Commit or stash the current target changes first${preview ? `: ${preview}` : ""}. Use --pro only when you intend to combine the changes.`,
    );
  }
}
...
async function approveNonGitInstall(action: string, targetRoot: string): Promise<boolean> {
  ...
  try {
    const answer = await prompt.question(`${action} target ${targetRoot} is not in Git. Initialize and commit it first, or type "continue" to approve this untracked install: `);
...
async function inspectGitCheckpoint(targetRoot: string): Promise<GitCheckpointState> {
  ...
  if (status.error || status.exitCode !== 0) {
    return {
      kind: "error",
      message: status.error?.message ?? (status.stderr.trim() || `git status exited ${status.exitCode}`),
    };
  }
...
async function assertPlannedFilesAreGitVisible(targetRoot: string, files: string[]): Promise<void> {
  ...
  if (ignoredFiles.length > 0) {
    throw new Error(
      `Git ignores planned install file${ignoredFiles.length === 1 ? "" : "s"}: ${ignoredFiles.join(", ")}. The installed files must be visible in Git for review. Change the ignore rules or rerun with --pro.`,
    );
  }
}
...
async function assertCoreCheckpointTracked(targetRoot: string): Promise<void> {
  ...
  const anchors = [path.join(targetRoot, "AGENTS.md"), path.join(targetRoot, agentsDirectoryName, "loader.md")].map((file) => toPosix(path.relative(state.root, file)));
  ...
  if (tracked.error || tracked.exitCode !== 0) {
    throw new Error(
      "Extension installation requires committed Core files. Review and commit AGENTS.md and .agents/loader.md before installing extensions. Use --pro only to bypass this checkpoint intentionally.",
    );
  }
}
...
async function printPostInstallCheckpoint(targetRoot: string, label: string, proMode: boolean): Promise<void> {
  ...
  if (state.kind === "repo" && !state.changes) {
    console.log(`${label}: Git reports no target changes. No new commit is needed.`);
    return;
  }
  const suffix = proMode ? " --pro bypassed the pre-install checkpoint." : "";
  console.log(`${label}: review the resulting diff and commit it before the next install.${suffix}`);
}
...
async function createCoreInstallPlan(sourceFiles: string[], targetRoot: string, catalogue: ShippedEntrypointCatalogue, preserveMissingDefaults: boolean): Promise<CoreInstallPlan> {
  ...
  const add = async (relativePath: string, targetFile: string, content: Buffer): Promise<void> => {
    ...
    const status: ExtensionPlanStatus = originalContent == null ? "create" : originalContent.equals(content) ? "unchanged" : "update";
    entries.set(portableExtensionPathKey(relativePath), {
      relativePath,
      targetFile,
      content,
      originalContent,
  ...
  for (const sourceFile of sourceFiles) {
    ...
    const nextText = patchMarker
      ...
      : targetText == null
        ? sourceText
        : preserveLocalBlocks(sourceText, targetText);
...
async function readExtensionReceipt(targetRoot: string): Promise<ExtensionOwnershipReceipt> {
  ...
  for (const [file, value] of Object.entries(raw.files)) {
    ...
    files[file] = {
      sha256: value.sha256,
      owners: value.owners.map((owner, index) => assertReceiptId(owner, `${receiptFile} ${file}.owners[${index}]`)),
    };
  }
  const receipt: ExtensionOwnershipReceipt = {
    schema: extensionReceiptSchema,
    roots,
    extensions,
    files,
...
function normalizeExtensionReceipt(receipt: ExtensionOwnershipReceipt): ExtensionOwnershipReceipt {
  ...
  for (const file of Object.keys(receipt.files).sort()) {
    files[file] = {
      sha256: receipt.files[file].sha256,
      owners: [...new Set(receipt.files[file].owners)].sort(),
    };
  }
  return {
    schema: extensionReceiptSchema,
    roots: [...new Set(receipt.roots)].sort(),
    extensions,
    files,
  };
}
...
async function createExtensionReceiptPlanEntry(receipt: ExtensionOwnershipReceipt, targetRoot: string): Promise<ExtensionInstallPlanEntry | null> {
  ...
  const status: ExtensionPlanStatus = content == null ? "delete" : originalContent == null ? "create" : originalContent.equals(content) ? "unchanged" : "update";
  return { relativePath: extensionReceiptFileName, targetFile, content, originalContent, status };
}

async function finalizeExtensionReceiptDigests(receipt: ExtensionOwnershipReceipt, targetRoot: string): Promise<ExtensionOwnershipReceipt> {
...
async function applyExtensionTransaction(plan: ExtensionInstallPlanEntry[], nextReceipt: ExtensionOwnershipReceipt, targetRoot: string): Promise<number> {
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `Extension transaction failed (${error instanceof Error ? error.message : String(error)}) and rollback also failed (${rollbackError instanceof Error ? rollbackError.message : String(rollbackError)})`,
      );
    }
    throw error;
  }
...
async function createExtensionInstallPlan(extensions: ExtensionSource[], targetRoot: string, currentReceipt: ExtensionOwnershipReceipt): Promise<ExtensionTransactionPlan> {
    ...
    for (const sourceFile of files) {
      ...
      if (existing) {
        if (!existing.sourceContent.equals(sourceContent)) {
          throw new Error(
            `Extension file collision at ${relativePath}: ${extensionDisplayName(existing.extensions[0])} and ${extensionDisplayName(extension)} provide different content`,
          );
        }
  ...
  for (const candidate of sortedCandidates) {
    ...
    const status: ExtensionPlanStatus = currentContent == null ? "create" : currentContent.equals(nextContent) ? "unchanged" : "update";
    ...
    if (managedOwners.length > 0 && candidate.extensions.some((extension) => extension.id == null)) {
      throw new Error(
        `Extension file ${candidate.relativePath} is provided by both managed and unmanaged packages. Give every provider a stable manifest id or keep their payload paths distinct`,
      );
    }
    ...
    if (currentContent != null && managedOwners.length > 0 && !existingOwnership) {
      throw new Error(
        `Managed extension may not claim or replace unowned existing file ${candidate.relativePath}. Remove the unowned path after review or use an unmanaged direct overlay`,
      );
    }
    ...
    if (existingOwnership && status === "update" && existingOwnership.owners.some((owner) => !managedOwners.includes(owner))) {
      throw new Error(
        `Extension file ${candidate.relativePath} cannot be updated unless every existing owner participates and supplies the same bytes. Missing owners: ${existingOwnership.owners.filter((owner) => !managedOwners.includes(owner)).join(", ")}`,
      );
    }
...
async function validateExtensionIndexPreflight(plan: ExtensionInstallPlanEntry[], targetRoot: string): Promise<void> {
  ...
  if (await isDirectory(scanRoot)) {
    for (const file of await listFiles(scanRoot, isIndexFile, {
      ...
      indexes.set(portableExtensionPathKey(toPosix(path.relative(scanRoot, file))), {
        file,
        content: null,
      });
    }
...
async function assertNoRoutedDescendantsDependOnDeletedEntrypoints(plan: ExtensionInstallPlanEntry[], targetRoot: string): Promise<void> {
  const plannedEntryFor = (file: string): ExtensionInstallPlanEntry | undefined => plan.find((entry) => samePath(entry.targetFile, file));
  ...
  for (const host of plan.filter((entry) => entry.content == null && isIndexFile(entry.targetFile))) {
    ...
    for (const candidate of candidates.values()) {
      ...
      const directDependency =
        samePath(parent, directory) && (isIndexEntryFile(path.basename(candidate), path.basename(directory)) || samePath(candidate, overwriteCompanion(host.targetFile)));
      const childDependency =
        samePath(path.dirname(parent), directory) &&
        (categoryEntrypointNames(path.basename(parent)).includes(path.basename(candidate)) || (skillsRoute && skillEntrypointNames.includes(path.basename(candidate))));
...
function countExtensionPlanStatuses(plan: ExtensionInstallPlanEntry[]): Record<ExtensionPlanStatus, number> {
  const counts: Record<ExtensionPlanStatus, number> = {
    create: 0,
    update: 0,
    unchanged: 0,
    delete: 0,
...
function countExtensionPlanScopes(plan: ExtensionInstallPlanEntry[]): Record<ExtensionPlanScope, number> {
  const counts: Record<ExtensionPlanScope, number> = {
    routed: 0,
    baseline: 0,
    executable: 0,
    workspace: 0,
...
function classifyExtensionPlanScope(relativePath: string): ExtensionPlanScope {
  ...
  const rootEntryFiles = new Set([...rootEntryPatchMarkers.keys()].map((file) => (caseInsensitive ? file.toLowerCase() : file)));
...
function extensionSourceKey(extension: ExtensionSource): string {
  return extension.kind === "bundled" ? `bundled:${extension.id}` : `local:${pathIdentity(extension.packageRoot)}`;
}
...
async function resolveExtensionSource(value: string, bundledPackages: Map<string, BundledExtensionPackage>): Promise<ExtensionSource> {
  ...
  if (await isDirectory(localRoot)) {
    ...
    const isDependencyOnlyPack = !hasPayloadDirectory && manifest.dependencies.length > 0 && !(await hasLocalOverlayPayloadFiles(localRoot));
...
async function resolveBundledExtensionSource(id: string, context = "requested directly", bundledPackages?: Map<string, BundledExtensionPackage>): Promise<ExtensionSource> {
...
function extensionCatalogueGroup(
  extension: Pick<BundledExtensionInfo, "contents"> & {
    sourceGroup?: ExtensionCatalogueGroup | null;
  },
...
function compareBundledExtensionInfos(left: BundledExtensionInfo, right: BundledExtensionInfo): number {
  const group = extensionCatalogueGroupOrder.indexOf(extensionCatalogueGroup(left)) - extensionCatalogueGroupOrder.indexOf(extensionCatalogueGroup(right));
...
async function classifyExtensionPayloadContents(payloadRoot: string): Promise<ExtensionContentKind[]> {
  ...
  for (const file of files) {
    const relativePath = toPosix(path.relative(payloadRoot, file));
    assertPortablePayloadPath(relativePath, payloadRoot);
    kinds.add(classifyExtensionPayloadPath(relativePath));
  }

  return extensionContentKindOrder.filter((kind) => kinds.has(kind));
}

function classifyExtensionPayloadPath(relativePath: string): ExtensionContentKind {
  ...
  const segments = relativePath.normalize("NFC").toLowerCase().split("/");
  return (segments[0] === ".agents" ? routeKinds[segments[1] ?? ""] : undefined) ?? "other";
}

async function hasExtensionPayloadFiles(payloadRoot: string): Promise<boolean> {
  return (await isDirectory(payloadRoot)) && (await listFiles(payloadRoot, () => true, { rejectLinksAndSpecialEntries: true })).length > 0;
}
...
async function readExtensionManifest(packageRoot: string, fallbackName: string): Promise<ExtensionManifest> {
  ...
  const dependencies = !hasDependencies
    ? []
    : (rawDependencies as unknown[]).map((dependency, index) => {
...
function assertPortablePayloadPath(value: string, extension: string): void {
  const segments = assertPortableRelativePath(value, `Extension ${extension} payload path`);
  const comparable = segments.map((segment) => segment.normalize("NFC").toLowerCase());
  if (comparable[0] === agentsDirectoryName && comparable[1] === "workspace") {
    throw new Error(`Extension ${extension} payload path uses the retired Workspace primitive root .agents/workspace/. Use .agents/maps/ and #Map instead`);
  }
}
...
export function createExtensionSelectionState(extensions: readonly ExtensionDependencyInfo[], directIds: Iterable<string> = []): ExtensionSelectionState {
...
export function toggleExtensionSelection(extensions: readonly ExtensionDependencyInfo[], state: ExtensionSelectionState, id: string): ExtensionSelectionState {
...
export const cliTestInternals = Object.freeze({
  assertCoreMapTopologyState,
  assertPortableManifestPath,
  assertPortablePayloadPath,
  classifyExtensionPayloadPath,
...
async function selectBundledExtensionIds(): Promise<string[]> {
  ...
  if (!process.stdin.isTTY || !process.stdout.isTTY || typeof process.stdin.setRawMode !== "function") {
    throw new Error(`Interactive extension selection requires a TTY. Use ${cliName} extend --list or ${cliName} extend --ids <id[,id...]>.`);
  }
  ...
  const render = (): void => {
    ...
    for (const group of extensionCatalogueGroupOrder) {
      ...
      for (const extension of grouped) {
        ...
        const mark = selection.direct.has(extension.id) ? "[direct]" : selection.required.has(extension.id) ? "[required]" : "[ ]";
        catalogueLines.push(formatBundledExtensionLine(extension, `${pointer} ${mark}`));
      }
    }
    const lines = ["Select bundled Open Forge extensions. Space toggles, Enter installs, q cancels. Dependencies are locked while required.", ...catalogueLines];
...
async function load(loadArgs: string[]): Promise<void> {
  ...
  const scanRoot = (await isDirectory(path.join(targetRoot, agentsDirectoryName))) ? path.join(targetRoot, agentsDirectoryName) : targetRoot;
...
function parseLoadArgs(args: string[]): { output: LoadOutput; target: string } {
  const usage = `Usage: ${cliName} load [--bodies|--paths|--json] [target]`;
...
function parseChainArgs(args: string[]): ChainOptions {
  const usage = `Usage: ${cliName} chain <route> [--heading <title>] [--json] [target]`;
...
async function buildRouteChain(targetFileInput: string, targetRoot: string, scanRoot: string): Promise<Array<{ file: string; kind: ChainItem["kind"]; overwriteOf?: string }>> {
  ...
  for (let index = 0; index < Math.max(0, segments.length - 1); index += 1) {
    ...
    throw new Error(`The \`route\` chain is discontinuous at ${workspaceRoute(targetRoot, current)}. Add a category \`entrypoint\` or run ${cliName} create category.`);
  }
...
function scanMarkdownHeadings(text: string): {
  lines: string[];
...
function parseFindArgs(findArgs: string[]): FindOptions {
  const usage = `Usage: ${cliName} find [--tag <Tag>]... [--route <path>] [--depth <n>] [--follow-required] [--bodies|--paths|--json] [target]`;
  ...
  if (options.depth > 0 && !options.route) {
    throw new Error(`${cliName} find: --depth requires --route`);
  }
...
async function resolveRouteArg(routeArg: string, targetRoot: string, scanRoot: string): Promise<string> {
  ...
  const candidates = [path.resolve(targetRoot, normalized), path.resolve(scanRoot, normalized)];
...
function normalizeDocumentRelativeRoute(routeArg: string, allowParentTraversal: boolean): string {
  const value = routeArg.trim();
  if (!value || value.includes("\0") || value.includes("\\") || path.isAbsolute(value) || path.win32.isAbsolute(value)) {
    throw new Error(`Route must be a document-relative Markdown path: ${routeArg}`);
  }

  const normalized = value.replace(/^(?:\.\/)+/, "");
  const segments = normalized.split("/");
  if (segments.some((segment) => !segment || segment === "." || segment.includes(":")) || (!allowParentTraversal && segments.includes(".."))) {
    throw new Error(`Route must not escape or change its Markdown document path: ${routeArg}`);
  }
  return normalized;
}

async function resolveRequiredRouteFile(route: ParsedRouteReference, sourceFile: string, targetRoot: string): Promise<string> {
...
async function resolveGeneratedRouteFile(entry: ParsedRouteReference, sourceFile: string, targetRoot: string): Promise<string> {
  const normalized = entry.syntax === "legacy-backtick" ? normalizeSafeRelativeRoute(entry.path) : normalizeDocumentRelativeRoute(entry.path, false);
  const candidate =
    entry.syntax === "legacy-backtick" && normalized.startsWith(`${agentsDirectoryName}/`)
...
function readGeneratedEntries(text: string): GeneratedEntry[] {
  return parseGeneratedEntries(text).map(({ path: entryPath, tags }) => ({
    path: entryPath,
    tags,
  }));
}
...
function findMatchingMarkdownDelimiter(value: string, openIndex: number, openCharacter: string, closeCharacter: string): number {
...
function decodeMarkdownRouteHref(destination: string): string | null {
  if (!destination || findUnescapedCharacter(destination, "#", 0) !== -1 || findUnescapedCharacter(destination, "?", 0) !== -1) {
    return null;
  }
...
function readTrailingTagSuffix(suffix: string): string[] {
  const match = suffix.match(/(?:^|\s+-\s+)(#[A-Za-z][A-Za-z0-9-]*(?:\s+#[A-Za-z][A-Za-z0-9-]*)*)\s*$/);
  return match ? [...match[1].matchAll(/#([A-Za-z][A-Za-z0-9-]*)/g)].map((tag) => tag[1]) : [];
}
...
function parseRequiredRoutes(rawText: string): ParsedRequiredRoutes {
  const result: ParsedRequiredRoutes = {
    paths: [],
    routes: [],
    none: false,
    invalid: [],
...
type GeneratedRegion = { status: "ok"; body: string } | { status: "none" } | { status: "malformed"; reason: string };
...
function readGeneratedRegion(text: string): GeneratedRegion {
  ...
  if (startMarkers.length !== 1 || endMarkers.length !== 1) {
    return {
      status: "malformed",
      reason: "the generated index markers are incomplete or duplicated",
    };
  }
...
async function doctor(doctorArgs: string[]): Promise<void> {
  const usage = `Usage: ${cliName} doctor [--json] [target]`;
  let json = false;
  let positional: string | null = null;
  ...
  const emit = (): void => {
    ...
    if (json) {
      ...
      console.log(`${cliName} doctor: no problems found in ${targetRoot}`);
    } else {
      for (const finding of findings) {
        console.log(`${finding.level}: ${finding.route} - ${finding.message}`);
      }
      console.log(`${cliName} doctor: ${errors.length} error(s), ${findings.length - errors.length} warning(s)`);
    }
  ...
  for (const owner of [...((await isFile(loaderFile)) ? [loaderFile] : []), ...regionOwners]) {
    ...
    if (region.status === "none") {
      report("warning", owner, `no generated index region. Run ${cliName} index`);
      continue;
    }

    const expected = samePath(owner, loaderFile) ? await computeLoaderBody(scanRoot) : await computeIndexBody(owner, path.dirname(owner));
    if (region.body !== expected.trim()) {
      report("warning", owner, `generated region is stale. Run ${cliName} index`);
    }
  ...
  for (const file of markdownFiles) {
    ...
    if (required.present) {
      ...
      for (const requiredRoute of required.routes) {
        try {
          ...
          if (validatesAsWorkflow) {
            for (const finding of await validateWorkflowRequiredRoute(requiredRoute, resolved, scanRoot, routedSet)) {
      ...
      for (const invalid of required.invalid) {
        report(validatesAsWorkflow ? "error" : "warning", file, `Required Routes line is not in entry format: ${invalid}`);
      }
    }
...
async function inferPrimitiveTypeFromRoute(file: string, scanRoot: string, explicitFileTags?: string[]): Promise<"workflow" | "directive" | null> {
  ...
  if (["pattern", "guidance", "skill", "template", "map", "memory"].some((tag) => declaredFileTypes.includes(tag))) return null;
...
async function validateWorkflowRequiredRoute(route: ParsedRouteReference, resolved: string, scanRoot: string, routedSet: Set<string>): Promise<string[]> {
  ...
  const declaredTypes = inheritedType ? [inheritedType] : declaredRouteTypeTags(readMetadata(await fs.readFile(resolved, "utf8")).tags);
...
function declaresWorkflowContract(text: string): boolean {
  return ["Goal", "Required Routes", "Steps", "Completion"].some((heading) => readMarkdownHeadingSections(text, heading).length > 0);
}
...
function validateDirectiveDocument(text: string, entrypoint: boolean): string[] {
  ...
  const instructions = readMarkdownHeadingSections(text, "Instructions");
  if (instructions.length !== 1 || !instructions[0].body.trim()) {
    findings.push("direct directive must declare exactly one non-empty Instructions section");
    return findings;
  }
  if (instructions[0].level !== 2) {
    findings.push("direct directive Instructions section must use a level-2 Markdown heading");
  }
  if (readMarkdownHeadingSections(text, "Axioms").length > 0) {
    findings.push("direct directive must use Instructions instead of Axioms");
  }
  if (hasDeclaredSentinel(instructions[0].body, "none") || hasDeclaredSentinel(instructions[0].body, "inherited")) {
    findings.push("Direct Directive Instructions must be substantive");
  }
  return findings;
}
...
async function create(createArgs: string[]): Promise<void> {
  ...
  if (kind === "category") {
    const usage = `Usage: ${cliName} create category <route-path> [target]`;
  ...
  if (kind === "extension") {
    const usage = `Usage: ${cliName} create extension <id> [directory]`;
    assertNoExtraArgs(createArgs, 3, usage);
    assertPositionalValue(createArgs[2], usage);
    await createExtensionScaffold(createArgs[1], createArgs[2] ?? process.cwd());
    return;
  }
  throw new Error(`Usage: ${cliName} create category <route-path> [target] | ${cliName} create extension <id> [directory]`);
}

async function createCategoryRoute(routeArg: string | undefined, targetArg: string): Promise<void> {
  if (!routeArg) {
    throw new Error(`Usage: ${cliName} create category <route-path> [target]`);
  }
  ...
  console.log(`Rebuilt ${generatedRegions} generated regions. Fill in the TODO descriptions, then run ${cliName} index again.`);
}
...
async function createExtensionScaffold(idArg: string | undefined, directoryArg: string): Promise<void> {
  if (!idArg || !isBundledExtensionId(idArg)) {
    throw new Error(`Usage: ${cliName} create extension <id> [directory]. \`id\` values are lowercase kebab-case such as my-patterns`);
  }
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify({ id: idArg, name, description: `TODO - one line shown by ${cliName} extend --list`, version: "0.1.0", dependencies: [] }, null, 2)}\n`,
  );
  await fs.writeFile(
...
\`\`\`sh
${cliName} extend ${idArg} <target>
\`\`\`
`,
  );

  console.log(`Created extension scaffold at ${baseDirectory}`);
  console.log(`Add routed files under payload/.agents/, then install with ${cliName} extend.`);
}
  ...
  return (
    catalogue.templates.find(
      (template) =>
        ...
        matchesManagedShippedEntrypointShape(targetSegments, template, catalogue.managedFoldersByRoot.get(template.rootFolder) ?? new Set<string>()),
    ) ?? null
  );
}

function matchesManagedShippedEntrypointShape(targetSegments: string[], template: ShippedEntrypointTemplate, managedFolderNames: Set<string>): boolean {
  ...
  const targetManagedSegments = targetFolders.slice(1).filter((segment) => managedFolderNames.has(segment));
  return targetManagedSegments.length === template.managedSegments.length && targetManagedSegments.every((segment, index) => segment === template.managedSegments[index]);
}
...
async function findCategoryEntrypoint(directory: string): Promise<string | null> {
  ...
  if (matches.length > 1) {
    throw new Error(
      `Multiple category entrypoints found in ${directory}: ${matches
...
async function findSkillEntrypoint(directory: string): Promise<string | null> {
  ...
  const matches = entries.filter((entry) => entry.isFile() && skillEntrypointNames.includes(entry.name)).map((entry) => path.join(directory, entry.name));

  if (matches.length > 1) {
    throw new Error(
      `Multiple skill entrypoints found in ${directory}: ${matches
        .map((file) => path.basename(file))
        .sort()
        .join(", ")}. Keep exactly one.`,
    );
...
function isIndexEntryFile(name: string, folderName: string): boolean {
  return name.endsWith(".md") && !isOverwriteCompanionPath(name) && !skillEntrypointNames.includes(name) && !categoryEntrypointNames(folderName).includes(name);
}
...
function patchMarkedBlock(targetText: string | null, sourceText: string, markerName: string, relativePath: string): string {
  ...
  const separator = targetText.length === 0 ? "" : targetText.endsWith("\n\n") ? "" : targetText.endsWith("\n") ? "\n" : "\n\n";
  return `${targetText}${separator}${sourceBlock.text}\n`;
}

function locateManagedBlock(text: string, markerName: string, context: string, required: true): ManagedBlockLocation;
function locateManagedBlock(text: string, markerName: string, context: string, required: false): ManagedBlockLocation | null;
function locateManagedBlock(text: string, markerName: string, context: string, required: boolean): ManagedBlockLocation | null {
  ...
  if (starts.length !== 1 || ends.length !== 1 || start == null || endStart == null || endStart <= start) {
    const validShape = required ? `exactly one complete ordered ${markerName} marker pair` : `either no ${markerName} markers or exactly one complete ordered marker pair`;
...
async function listFiles(
  ...
  options: {
    rejectLinksAndSpecialEntries?: boolean;
    rejectGitControlEntries?: boolean;
    entryContext?: string;
  } = {},
): Promise<string[]> {
...
function resolveRepoRoot(entryDirectory: string): string {
  const candidates = [path.resolve(entryDirectory, ".."), path.resolve(entryDirectory, "../..")];
...
function resolveFrameworkSourceRoot(repoRoot: string, entryDirectory: string): string {
  const candidates = [path.join(entryDirectory, "open-forge-src"), path.join(repoRoot, "src", "open-forge")];
...
function resolveBundledExtensionsRoot(repoRoot: string, entryDirectory: string): string {
  const candidates = [path.join(entryDirectory, "extensions"), path.join(repoRoot, "src", "extensions")];
...
Usage:
  ${cliName} install [target] [--pro]
  ${cliName} extend [--dry-run] [--pro]
  ${cliName} extend --list
  ${cliName} extend --select [target] [--dry-run] [--pro]
  ${cliName} extend --ids <id[,id...]> [target] [--dry-run] [--pro]
  ${cliName} extend --remove <id[,id...]> [target] [--dry-run] [--pro]
  ${cliName} extend <extension-source-or-id> [target] [--dry-run] [--pro]
  ${cliName} index [target]
  ${cliName} load [--bodies|--paths|--json] [target]
  ${cliName} find [--tag <Tag>]... [--route <path>] [--depth <n>] [--follow-required] [--bodies|--paths|--json] [target]
  ${cliName} chain <route> [--heading <title>] [--json] [target]
  ${cliName} doctor [--json] [target]
  ${cliName} create category <route-path> [target]
  ${cliName} create extension <id> [directory]
  ${cliName} help
  ${cliName} --help
  ${cliName} -h

Commands:
  install  Install Core after a clean Git checkpoint. --pro bypasses the Git and Core checkpoints.
  extend   Install or remove Extensions and their dependencies. --dry-run previews changes; --pro bypasses checkpoints.
  index    Rebuild generated route entries.
  load     Print the Loader, #LoadNow context, #KeepInMind context, and local overwrites.
  find     List routed files by tag or route. --bodies prints content; --follow-required includes Required Routes.
  chain    Show inherited routes from the Loader to a target, optionally limited to one Markdown heading.
  doctor   Check routes, Workflows, Directives, generated entries, and dependencies.
  create   Create a category route or Extension package.
