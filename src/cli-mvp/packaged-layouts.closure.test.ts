...
import { repoPath, repoRoot, runProcess, useTestSandbox, writeText } from "../../tests/support/index.ts";

const cliSource = repoPath("src", "cli-mvp", "cli.ts");
...
describe("packaged bundled-extension resolution", () => {
  ...
  test("discovers a nested npm-style package by manifest id rather than folder name", async () => {
    ...
    await writeText(path.join(packageRoot, "src", "open-forge", "AGENTS.md"), "# Fixture payload\n");
    await writeFixtureExtension(path.join(packageRoot, "src", "extensions", "support"), extensionId, marker, "physical-folder-name");
...
async function buildPackagedCli(packageRoot: string): Promise<string> {
  ...
  const result = await runProcess([process.execPath, "build", cliSource, "--target=node", "--format=esm", `--outfile=${cliFile}`], { cwd: repoRoot });
