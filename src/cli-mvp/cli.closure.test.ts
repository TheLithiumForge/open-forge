...
const cliFile = repoPath("src", "cli-mvp", "cli.ts");
...
describe("MVP command dispatch", () => {
  test("prints help explicitly and rejects unknown commands", async () => {
    ...
    expect(defaultHelp.stdout).toContain(`${cliName} help`);
    expect(defaultHelp.stdout).toContain(`${cliName} --help`);
    expect(defaultHelp.stdout).toContain(`${cliName} -h`);
  ...
  test("rejects invalid mutation arguments before changing the target", async () => {
    ...
    expect(indexResult.stderr).toContain(`Usage: ${cliName} index [target]`);
    expect(createResult.exitCode).toBe(1);
    expect(createResult.stderr).toContain(`Usage: ${cliName} create category <route-path> [target]`);
    expect(createOptionResult.exitCode).toBe(1);
    expect(createOptionResult.stderr).toContain(`Usage: ${cliName} create category <route-path> [target]`);
    expect(extendOptionResult.exitCode).toBe(1);
    expect(extendOptionResult.stderr).toContain(`Usage: ${cliName} extend <extension-source-or-id> [target]`);
    expect(installResult.exitCode).toBe(1);
    expect(installResult.stderr).toContain(`Usage: ${cliName} install [target] [--pro]`);
...
`,
    ...
    await writeRoute(category, "repositories.md", "Repository routes", ["Map", "Route"]);

    const result = await runCli("index", root);
    const content = await fs.readFile(path.join(category, "_maps.md"), "utf8");

    expect(result.exitCode).toBe(0);
    expect(content).toContain("Stable category contract.");
    expect(content).toContain("<!-- open-forge:generated-index:start -->");
    expect(content).toContain("- [Repository routes](repositories.md) - #Map #Route");
...
`;
  ...
  test("validates every generated index before writing any of them", async () => {
    const root = await createRoot();
    const valid = await createCategory(
      root,
      "alpha",
      `# Alpha
...
`,
  ...
  test("routes categories through child entrypoints at arbitrary depth", async () => {
    const root = await createRoot();
    const maps = await createCategory(root, "maps", "# Maps\n");
    const repositories = await createCategory(maps, "repositories", "# Repositories\n");
    ...
    const result = await runCli("index", root);
    const parent = await fs.readFile(path.join(maps, "_maps.md"), "utf8");
  ...
  test("indexes native skill packages under skills routes", async () => {
    ...
    const skills = await createCategory(
      agents,
      "skills",
      `---
open-forge:
...
`,
    ...
    await fs.writeFile(
      path.join(implementation, "SKILL.md"),
      `---
name: implementation
...
`,
  ...
  test("escapes link labels, encodes route segments, and follows the decoded generated route", async () => {
    ...
    expect(index).toContain("- [Use \\[draft\\] \\\\ path #NotATag](route%20%28draft%29%20%231%25%20ready.md) - #Guide #Actual");
...
describe("Framework source registry", () => {
  test("keeps every shipped .agents contract aligned with dogfood authored content", async () => {
    ...
    for (const [relativePath, encodedSource] of Object.entries(source)) {
      ...
      expect(sourceContent.replace(generatedRegion, "<generated Entries>")).toBe(dogfoodContent.replace(generatedRegion, "<generated Entries>"));
    }
  });
});
...
describe("loader category registry", () => {
  test("generates direct active categories from their entrypoint metadata", async () => {
    const root = await createRoot();
    await fs.writeFile(
      path.join(root, "loader.md"),
      `# Loader

Stable loading contract.
`,
    );
    const maps = await createCategory(
      root,
      "maps",
      `---
open-forge:
  description: Maps to important project locations and when to use them
  tags: [LoadNow, Map]
---
...
`,
    ...
    await createCategory(maps, "repositories", "# Repositories\n");
    ...
    expect(loader).toContain("- [Maps to important project locations and when to use them](maps/_maps.md) - #LoadNow #Map");
  ...
  for (const alias of ["index.md", "_index.md", "references.md", "_references.md"]) {
    test(`supports ${alias} as a compatibility category entrypoint`, async () => {
      ...
      const external = await createCategory(
        root,
        "external",
        `---
...
describe("install", () => {
  test("installs a structurally valid and idempotent Core route tree", async () => {
    ...
    expect(result.exitCode).toBe(0);
    expect(await exists(path.join(root, ".agents", "maps", "_maps.md"))).toBe(true);
    expect(await exists(path.join(root, ".agents", "workspace"))).toBe(false);
    ...
    expect(installedLoader).toContain("#Evergreen - Material that must stay aligned with accepted current state");
    ...
    expect(installedLoader).toContain("Any number of routed `slugs` may narrow a route below its root");
    expect(installedLoader).toContain("keeps their segment order through scopes");
    expect(installedLoader).toContain("A familiar `slug` or tag does not create root behavior or managed status");
    expect(installedLoader).toContain("Each manager defines the route shapes it recognizes");
    expect(installedLoader).toContain("Other valid routes remain routable");
    expect(installedLoader).toContain("Follow relevant branches recursively");
    expect(installedLoader).toContain("companion loads immediately after `{name}.md`");
    expect(installedLoader).toContain("It is not indexed or selected separately.");
    expect(installedLoader).toContain("When a base file and overwrite answer the same question differently, the overwrite wins only within the base file's scope.");
    expect(installedLoader).toContain("Tags change what loads, when it loads, or how it is classified. They do not create authority.");
    expect(installedLoader).toContain("A request to act allows the agent to make the routine, reversible, in-scope choices needed to complete it.");
    expect(installedLoader).not.toContain("A request to act also accepts any decision required to perform that action");
    expect(installedLoader).toContain("Follow clear user direction when it is safe and allowed. It sets the task's goals, priorities, important choices, and accepted changes.");
    expect(installedLoader).not.toContain("Clear user direction governs goals, priorities, consequential tradeoffs, and accepted changes within its stated scope");
    expect(installedLoader).toContain("Only this loader and recognized loaded `entrypoints` can define active `Axioms`.");
    expect(installedLoader).toContain("Accepted workspace-specific state replaces Open Forge defaults in that scope.");
    expect(installedLoader).not.toContain("Only `Axioms` in this loader or a recognized `entrypoint` are active.");
    expect(installedLoader).toContain("Loaded binding instructions add to one another");
    expect(installedLoader).toContain("Tagged entrypoints: Read one proactively only when initial loader or #LoadNow traversal exposes it");
    expect(installedLoader).toContain("Other tagged files: Read every routed #KeepInMind file that is not an `entrypoint` across the workspace");
    expect(installedLoader).toContain("Order: Read missing ancestor `entrypoints` first.");
    expect(installedLoader).toContain("at each required #KeepInMind boundary");
    expect(installedLoader).toContain("Update affected #Evergreen material you may edit");
    expect(installedLoader).toContain("#Memory - Self-growing Markdown state");
    ...
    const generatedRegion = /<!-- open-forge:generated-index:start -->[\s\S]*?<!-- open-forge:generated-index:end -->/;
    expect(installedMemory).toContain("Memory is self-growing Markdown state");
    expect(installedMemory).toContain("without a fixed structural ceiling");
    expect(installedMemory).toContain("unrelated branches stay outside active context");
    expect(installedMemory).toContain("Growth is deliberate rather than automatic");
    expect(installedMemory).toContain("Do not save every conversation");
    expect(installedMemory).toContain("Memory may record any subject, including how work is performed");
    expect(installedMemory.replace(generatedRegion, "<generated Entries>")).toBe(sourceMemory.replace(generatedRegion, "<generated Entries>"));

    const installedCheckpoint = await fs.readFile(path.join(root, ".agents", "memory", "working", "checkpoints", "_checkpoints.md"), "utf8");
    const installedHandoff = await fs.readFile(path.join(root, ".agents", "memory", "working", "handoffs", "_handoffs.md"), "utf8");
    expect(installedCheckpoint).toContain("stable boundary snapshot");
    expect(installedHandoff).toContain("actual transfer or explicitly planned resumption");
    expect(installedHandoff).toContain("Do not edit a sealed Handoff");

    const installedDocuments = await fs.readFile(path.join(root, ".agents", "memory", "crystallized", "documents", "_documents.md"), "utf8");
    ...
    expect(installedDirectives).not.toContain("writing.md");

    const installedTemplates = await fs.readFile(path.join(root, ".agents", "templates", "_templates.md"), "utf8");
    expect(installedTemplates).toContain("Templates are reusable starting files. Copy and adapt them, then maintain the result independently.");
    expect(installedTemplates).toContain("tags: [Core, Template]");
    expect(installedTemplates).toContain("Link a matching #Core route");
    expect(installedTemplates).toContain("State the need and primary question each Template answers");
    ...
    expect(keepInMindRoutes).toEqual(expect.arrayContaining([".agents/memory/emerging/_emerging.md", ".agents/memory/emerging/observations/_observations.md"]));
  ...
  test("stops before writing when the retired Workspace primitive root is present", async () => {
    const root = await createRoot();
    await createCategory(path.join(root, ".agents"), "workspace", "# Legacy Workspace\n");
    const before = await snapshotTreeState(root);

    const result = await runCliDefault("install", root);

    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain("former Workspace primitive at .agents/workspace/");
    expect(await snapshotTreeState(root)).toEqual(before);
  });

  test("stops before writing when legacy and current Map roots coexist", async () => {
    const root = await createRoot();
    const agents = path.join(root, ".agents");
    await createCategory(agents, "workspace", "# Legacy Workspace\n");
    await createCategory(agents, "maps", "# Maps\n");
    const before = await snapshotTreeState(root);

    const result = await runCliDefault("install", root);

    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain("both .agents/workspace/ and .agents/maps/");
    expect(await snapshotTreeState(root)).toEqual(before);
  });

  test("stops before writing when a Map root has no recognizable Core owner", async () => {
    const root = await createRoot();
    await createCategory(path.join(root, ".agents"), "maps", "# User Map\n");
    const before = await snapshotTreeState(root);

    const result = await runCliDefault("install", root);

    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain("without recognizable Core anchors");
    expect(await snapshotTreeState(root)).toEqual(before);
  });
  ...
  test("reconciles Open Forge-managed route entrypoints through scopes", async () => {
    ...
    await fs.writeFile(
      path.join(scopedDocuments, "_documents.md"),
      `# Old Scoped Documents
...
`,
    ...
    await fs.writeFile(
      path.join(scopedTemplates, "_templates.md"),
      `# Old Scoped Templates
...
`,
    ...
    expect(result.exitCode).toBe(0);
    expect(scopedContent).toContain("Documents explain accepted current knowledge that needs one coherent view");
    ...
    expect(scopedTemplateContent).not.toContain("Templates are reusable starting files");
  });
  ...
  test("stops before repairing an ambiguous Map loader entry", async () => {
    const root = await createRoot();
    expect((await runCli("install", root)).exitCode).toBe(0);
    const loader = path.join(root, ".agents", "loader.md");
    await fs.writeFile(loader, (await fs.readFile(loader, "utf8")).replace(generatedIndexEndMarker, "<!-- damaged-generated-index:end -->"));
    const before = await snapshotTreeState(root);

    const result = await runCli("install", root);

    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain("without a recognized #Map entrypoint and loader entry");
    expect(await snapshotTreeState(root)).toEqual(before);
  });

  test("preserves a user-owned legacy Sessions route byte-identically during reinstall", async () => {
    const root = await createRoot();
    expect((await runCli("install", root)).exitCode).toBe(0);

    const sessions = path.join(root, ".agents", "memory", "working", "sessions");
    const legacy = `---
open-forge:
  description: Legacy user-owned chronological work history
  tags: [Memory, Session, Contextual]
---

# Sessions

User-owned legacy content remains available.
`;
    await fs.mkdir(sessions, { recursive: true });
    await fs.writeFile(path.join(sessions, "_sessions.md"), legacy);

    expect((await runCli("install", root)).exitCode).toBe(0);
    expect(await fs.readFile(path.join(sessions, "_sessions.md"), "utf8")).toBe(legacy);
  });
  ...
  test("does not treat arbitrary routed scopes as Open Forge-managed routes", async () => {
    ...
    await fs.writeFile(
      path.join(reactPatterns, "_react.md"),
      `# React
...
`,
    ...
    expect(reactContent).not.toContain("Reusable default shapes");
  ...
  test("installs a local extension overlay and rebuilds indexes", async () => {
    ...
    await fs.writeFile(
      path.join(extensionPatterns, "_react.md"),
      `---
open-forge:
...
`,
  ...
  test("installs a bundled first-party extension by id", async () => {
    ...
    await fs.mkdir(extensionPatterns, { recursive: true });
    await fs.writeFile(
      path.join(extensionsRoot, "review-pack", "extension.json"),
      JSON.stringify({
        id: "review-pack",
        name: "Review Pack",
    ...
    await fs.writeFile(
      path.join(extensionPatterns, "_reviews.md"),
      `---
open-forge:
  description: Review patterns bundled with Open Forge
...
`,
  ...
  test("discovers nested bundled packages by stable manifest id instead of folder name", async () => {
    ...
    const installed = await runCliWithEnv("extend", ["stable-package-id", root], {
      OPEN_FORGE_EXTENSIONS_ROOT: extensionsRoot,
    });
  ...
  test("derives every advertised content kind from payload paths", async () => {
    ...
    const files = [
      ...
      path.join(agents, "maps", "mixed.md"),
    ...
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain("(contents: skill, workflow, directive, guidance, pattern, template, map, memory, other)");
  });

  test("rejects Extension source under the retired Workspace primitive root", async () => {
    const extensionsRoot = await createRoot();
    const patterns = await createBundledExtension(extensionsRoot, "legacy-map-pack", "Legacy Map Pack", "Legacy Map extension");
    const payload = path.resolve(patterns, "..", "..", "..");
    const legacy = path.join(payload, ".agents", "workspace", "routes.md");
    ...
    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain("retired Workspace primitive root");
  });
  ...
  test("deduplicates byte-identical files shared by extensions", async () => {
    ...
    expect(
      (
        await runCliWithEnv("extend", ["--ids", "alpha-pack,beta-pack", root], {
          OPEN_FORGE_EXTENSIONS_ROOT: extensionsRoot,
        })
      ).exitCode,
    ).toBe(0);
    ...
    expect(
      (
        await runCliWithEnv("extend", ["alpha-pack", root], {
          OPEN_FORGE_EXTENSIONS_ROOT: extensionsRoot,
        })
      ).exitCode,
    ).toBe(0);
  ...
  test("preserves dependency-only pack semantics when installing a copied local pack", async () => {
    ...
    await createBundledExtension(extensionsRoot, "shared-pack", "Shared Pack", "Shared extension");
    await fs.writeFile(
      path.join(copiedPack, "extension.json"),
      `${JSON.stringify(
        {
  ...
  test("reconciles whole-file payload paths dropped by a managed reinstall", async () => {
    ...
    expect(
      (
        await runCliWithEnv("extend", ["evolving-pack", root], {
          OPEN_FORGE_EXTENSIONS_ROOT: extensionsRoot,
        })
      ).exitCode,
    ).toBe(0);
    ...
    const updated = await runCliWithEnv("extend", ["evolving-pack", root], {
      OPEN_FORGE_EXTENSIONS_ROOT: extensionsRoot,
    });
  ...
  test("blocks dependent removal and locally modified owned files", async () => {
    ...
    expect(
      (
        await runCliWithEnv("extend", ["dependent-pack", root], {
          OPEN_FORGE_EXTENSIONS_ROOT: extensionsRoot,
        })
      ).exitCode,
    ).toBe(0);

    const blocked = await runCliWithEnv("extend", ["--remove", "base-routing", root], {
      OPEN_FORGE_EXTENSIONS_ROOT: extensionsRoot,
    });
    ...
    expect(
      (
        await runCliWithEnv("extend", ["--remove", "dependent-pack", root], {
          OPEN_FORGE_EXTENSIONS_ROOT: extensionsRoot,
        })
      ).exitCode,
    ).toBe(0);
    ...
    const modified = await runCliWithEnv("extend", ["--remove", "base-routing", root], {
      OPEN_FORGE_EXTENSIONS_ROOT: extensionsRoot,
    });
    ...
    const unrelated = await runCliWithEnv("extend", ["unrelated-pack", root], {
      OPEN_FORGE_EXTENSIONS_ROOT: extensionsRoot,
    });
  ...
  test("tracks shared file owners and refuses removal after an owned file is modified", async () => {
    ...
    expect(
      (
        await runCliWithEnv("extend", ["--ids=alpha-owner,beta-owner", root], {
          OPEN_FORGE_EXTENSIONS_ROOT: extensionsRoot,
        })
      ).exitCode,
    ).toBe(0);
    ...
    expect(
      (
        await runCliWithEnv("extend", ["--remove", "alpha-owner", root], {
          OPEN_FORGE_EXTENSIONS_ROOT: extensionsRoot,
        })
      ).exitCode,
    ).toBe(0);
    ...
    const modified = await runCliWithEnv("extend", ["--remove", "beta-owner", root], {
      OPEN_FORGE_EXTENSIONS_ROOT: extensionsRoot,
    });
    expect(modified.exitCode).toBe(1);
  ...
  test("fails closed on duplicate and cross-inconsistent ownership receipts", async () => {
    ...
    expect(
      (
        await runCliWithEnv("extend", ["managed-pack", root], {
          OPEN_FORGE_EXTENSIONS_ROOT: extensionsRoot,
        })
      ).exitCode,
    ).toBe(0);
    ...
    const duplicate = await runCliWithEnv("extend", ["--remove", "managed-pack", root], {
      OPEN_FORGE_EXTENSIONS_ROOT: extensionsRoot,
    });
    ...
    await fs.writeFile(receiptFile, `${JSON.stringify(validReceipt, null, 2)}\n`);
    const inconsistent = await runCliWithEnv("extend", ["--remove", "managed-pack", root], {
      OPEN_FORGE_EXTENSIONS_ROOT: extensionsRoot,
    });
  ...
  test("migrates empty schema-1 augmentation arrays but rejects retained legacy augmentation state", async () => {
    ...
    expect(
      (
        await runCliWithEnv("extend", ["legacy-pack", root], {
          OPEN_FORGE_EXTENSIONS_ROOT: extensionsRoot,
        })
      ).exitCode,
    ).toBe(0);
    ...
    const removed = await runCliWithEnv("extend", ["--remove", "legacy-pack", root], {
      OPEN_FORGE_EXTENSIONS_ROOT: extensionsRoot,
    });
    ...
    const rejected = await runCliWithEnv("extend", ["--remove", "legacy-pack", root], {
      OPEN_FORGE_EXTENSIONS_ROOT: extensionsRoot,
    });
  ...
  test("does not catalogue or install a manifest-only no-op extension", async () => {
    ...
    await fs.mkdir(path.join(extensionsRoot, "empty-pack", "payload", ".agents"), {
      recursive: true,
    });
...
describe("Git-checkpointed install commands", () => {
  test("fails closed outside Git in noninteractive mode without mutating the target", async () => {
    ...
    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain("requires a Git repository so the changes can be reviewed");
  ...
  test("uses Core and each extension transaction as real Git review checkpoints", async () => {
    ...
    expect(core.stdout).toContain(`${cliName} extend --list`);

    const blocked = await runCliDefault("extend", extension, root);
    expect(blocked.exitCode).toBe(1);
    expect(blocked.stderr).toContain("requires a clean Git state");
    ...
    expect(installed.exitCode).toBe(0);
    expect(installed.stdout).toContain("Extension transaction: review the resulting diff and commit it before the next install");
    ...
    expect(nextBlocked.stderr).toContain("requires a clean Git state");
  });
...
`;
    ...
    expect(agents).toContain("Open Forge provides the working rules and context for this workspace.");
    expect(agents).toContain("Before starting a task, read `.agents/loader.md`.");
    expect(agents).toContain("Use it to select every relevant scope, including nested scopes.");
    expect(agents).toContain("Follow the loaded rules throughout the task.");
    expect(agents).not.toContain("Before acting on any task, you must read `.agents/loader.md`");
    ...
    expect(claude.match(/@\.agents\/loader\.md/g) ?? []).toHaveLength(1);
...
`,
    ...
    const guides = await createCategory(
      root,
      "guides",
      `---
open-forge:
...
`,
  ...
  test("blocks invalid and empty Required Routes sections instead of silently skipping them", async () => {
    ...
    const workflows = await createCategory(root, "workflows", "# Workflows\n");
    await fs.writeFile(
      path.join(workflows, "invalid.md"),
      `# Invalid
...
`,
  ...
  test("rejects a linked routed category across find, chain, doctor, and index", async () => {
    ...
    await fs.writeFile(
      path.join(outsidePatterns, "_patterns.md"),
      `---
open-forge:
...
`;
  ...
  test("keeps legacy .agents-prefixed generated routes readable during migration", async () => {
    ...
    await fs.writeFile(path.join(agents, "loader.md"), routedDocument("Loader", ["Core"], ["- `.agents/directives/_directives.md` - Legacy root route - #LoadNow #Directive"]));
    ...
    expect(loaded.stdout.trim().split(/\r?\n/)).toEqual([".agents/loader.md", ".agents/directives/_directives.md"]);
  });
  ...
  test("emits loader-first visible LoadNow routes, adjacent overwrites, and complete KeepInMind context", async () => {
    ...
    await fs.writeFile(
      ...
      routedDocument(
        "Loader",
        ["Core"],
        ["- [Directives fixture](directives/_directives.md) - #LoadNow #Directive", "- [Evergreen current view](current-view.md) - #CurrentTruth #Evergreen"],
      ),
    );
    ...
    await fs.writeFile(path.join(nested, "_nested.md"), routedDocument("Nested Directives", ["Directive"], ["- [Nested direct directive](child-rule.md) - #LoadNow #Directive"]));
    await fs.writeFile(path.join(nested, "child-rule.md"), routedDocument("Child Rule", ["LoadNow", "Directive"]));
    await fs.writeFile(path.join(memory, "_memory.md"), routedDocument("Memory", ["KeepInMind", "Memory"], ["- [Remembered detail](detail.md) - #LoadNow #Memory"]));
    ...
    const exposedParent = (await fs.readFile(directivesIndex, "utf8")).replace(
      "#Directive\n<!-- open-forge:generated-index:end -->",
      "#LoadNow #Directive\n<!-- open-forge:generated-index:end -->",
    );
    await fs.writeFile(directivesIndex, exposedParent);
    const withNestedParent = await runCli("load", "--json", root);
    const items = JSON.parse(withNestedParent.stdout) as Array<{
      route: string;
      kind: string;
      companionOf?: string;
    }>;
...
describe("chain command", () => {
  test("prints loader-to-target heading inheritance with overwrite adjacency", async () => {
    ...
    const componentText = (await fs.readFile(componentEntrypoint, "utf8")).replace("- inherited - No local axioms. Loaded ancestor axioms remain active.", "- none");
    ...
    expect(parsed.chain.map((item) => item.heading.status)).toEqual(["content", "content", "declared-inherited", "content", "content", "content", "content"]);
  ...
  test("includes a native skill entrypoint before an internal skill resource", async () => {
    ...
    expect(routes).toEqual([".agents/loader.md", ".agents/skills/_skills.md", ".agents/skills/external/SKILL.md", ".agents/skills/external/references/check.md"]);
  });
...
describe("doctor command", () => {
  ...
  test("rejects none as a category Axioms sentinel", async () => {
    ...
    const text = (await fs.readFile(entrypoint, "utf8")).replace("- inherited - No local axioms. Loaded ancestor axioms remain active.", "- none");
...
`,
  ...
  test("rejects malformed, unrouted, and incorrectly typed Workflow dependencies", async () => {
    ...
    await fs.writeFile(
      path.join(workflows, "invalid-dependencies.md"),
      `---
open-forge:
...
`,
    ...
    expect(result.exitCode).toBe(1);
    expect(findingLevel("Required Routes line is not in entry format: - [Missing canonical tags](../patterns/delivery.md)")).toBe("error");
    expect(findingLevel("workflow Required Route is not reachable through generated routing: ../private/helper.md")).toBe("error");
    expect(findingLevel("workflow Required Route ../patterns/delivery.md must include its target primitive tag #Pattern")).toBe("error");
  });
...
`,
  ...
  test("validates a root Workflow entrypoint when it declares recipe sections", async () => {
    ...
    await fs.writeFile(entrypoint, content.replace("## Axioms", "## Goal\n\n- Declare a partial recipe.\n\n## Axioms"));
    expect((await runCli("index", root)).exitCode).toBe(0);
...
`,
  ...
  test("requires substantive level-2 Instructions on direct directive files", async () => {
    const root = await createRoot();
    expect((await runCli("install", root)).exitCode).toBe(0);
...
`,
    ...
    expect(result.exitCode).toBe(1);
    expect(result.stdout).toContain("direct directive must declare exactly one non-empty Instructions section");
  });
  ...
  test("infers workflow and directive validation from routed category ancestry", async () => {
    ...
    await fs.writeFile(
      path.join(root, ".agents", "directives", "tag-bypass.md"),
      `---
open-forge:
...
`,
  ...
  test("validates explicitly tagged primitives under a neutral route", async () => {
    ...
    await fs.writeFile(
      path.join(custom, "flow.md"),
      ["---", "open-forge:", "  description: Explicit workflow in a neutral route", "  tags: [Workflow]", "---", "", "# Flow", "", "## Steps", "", "1. Run.", ""].join("\n"),
    );
    await fs.writeFile(
      path.join(custom, "rule.md"),
      ["---", "open-forge:", "  description: Explicit directive in a neutral route", "  tags: [Directive]", "---", "", "# Rule", "", "No binding axioms were declared.", ""].join(
        "\n",
      ),
    );
    expect((await runCli("index", root)).exitCode).toBe(0);
  ...
  test("keeps nested slugs and conflicting tags inside the containing root route contract", async () => {
    ...
    await fs.writeFile(path.join(localPatterns, "_patterns.md"), localEntrypoint.replace("tags: [Workflow]", "tags: [Pattern]"));
    ...
    await fs.writeFile(path.join(localDirectives, "_local.md"), directiveEntrypoint.replace("tags: [Directive]", "tags: [Pattern]"));
...
`,
  ...
  test("does not infer route behavior from a familiar root entrypoint without a primitive tag", async () => {
    ...
    await fs.writeFile(
      path.join(workflows, "_workflows.md"),
      `---
open-forge:
...
describe("create command", () => {
  ...
  test("scaffolds an extension package", async () => {
    ...
    expect(metadata).toEqual({
      id: "my-pack",
      name: "My Pack",
      description: `TODO - one line shown by ${cliName} extend --list`,
      version: "0.1.0",
      dependencies: [],
    });
    expect(await fs.readFile(path.join(packageRoot, "README.md"), "utf8")).toContain(`${cliName} extend my-pack <target>`);

    const routeRoot = path.join(packageRoot, "payload", ".agents", "patterns", "my-pack");
    await fs.mkdir(routeRoot, { recursive: true });
    await fs.writeFile(
      path.join(routeRoot, "_my-pack.md"),
      `---
open-forge:
