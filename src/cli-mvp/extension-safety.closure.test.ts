...
import { pathExists, repoPath, requireSuccess, runCli as executeCli, snapshotTreeState, useTestSandbox, writeText } from "../../tests/support/index.ts";
...
describe("extension lifecycle safety closure", () => {
  test("unmanaged overlay cannot replace a receipt-owned file", async () => {
    ...
    await writeExtensionPackage(
      managed,
      { id: "managed-owner", name: "Managed Owner" },
      {
  ...
  test("extension removal rejects a linked target root", async () => {
    ...
    await writeExtensionPackage(
      managed,
      { id: "linked-owner", name: "Linked Owner" },
      {
  ...
  test("Core reinstall cannot change a receipt-owned file", async () => {
    ...
    await writeExtensionPackage(
      managed,
      { id: "loader-owner", name: "Loader Owner" },
      {
  ...
  test("direct bundled id rejects a linked catalogue package root", async () => {
    ...
    await writeExtensionPackage(
      outside,
      { id: "linked-pack", name: "Linked Pack" },
      {
    ...
    const result = await runCli(["extend", "linked-pack", target], {
  ...
  test("standalone index keeps managed entrypoint ownership usable around unmanaged routed children", async () => {
    ...
    await writeExtensionPackage(
      extension,
      { id: "managed-route", name: "Managed Route" },
      {
    ...
    await writeText(path.join(extension, "payload", ...entrypointRelative.split("/")), managedRouteEntrypoint("Authored contract v2."));
  ...
  test("Core index regeneration does not stale managed entrypoint ownership", async () => {
    ...
    await writeExtensionPackage(
      extension,
      { id: "core-managed-route", name: "Core Managed Route" },
      {
    ...
    await writeText(path.join(target, ".agents", "patterns", "core-managed-route", "local-note.md"), unmanagedRoutedChild());
    ...
    await writeText(path.join(extension, "payload", ...entrypointRelative.split("/")), managedRouteEntrypoint("Core-index contract v2."));
  ...
  test("removal preserves a managed entrypoint required by an unmanaged routed child", async () => {
    ...
    await writeExtensionPackage(
      extension,
      { id: "retained-route", name: "Retained Route" },
      {
  ...
  test("injected extension transaction failures restore payload index receipt and tree", async () => {
    ...
    await writeExtensionPackage(
      extension,
      { id: "transaction-pack", name: "Transaction Pack" },
      {
...
async function runCli(args: string[], env: Record<string, string> = {}) {
  const withPro = (args[0] === "install" || args[0] === "extend") && !args.includes("--pro") ? [...args, "--pro"] : args;
...
async function writeExtensionPackage(packageRoot: string, manifest: Record<string, unknown>, payloadFiles: Record<string, string> = {}): Promise<void> {
  await writeText(
