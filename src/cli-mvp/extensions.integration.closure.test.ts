...
const toolkitWorkflowFiles = ["architecture.md", "debugging.md", "development.md", "planning.md", "review.md", "vision.md"] as const;
...
describe("first-party extension integration", () => {
  ...
  test("installs the complete toolkit with one owner and valid routed contents", async () => {
    ...
    const installedWorkflowFiles = (await fs.readdir(workflowDirectory)).filter((name) => name !== "_workflows.md").sort();
    ...
    expect((await fs.readdir(path.join(skillRoot, "references"))).sort()).toEqual(["map-user-experience.md", "prepare-implementation-handoff.md", "review-experience-design.md"]);
    ...
    expect(await fs.readFile(path.join(root, ".agents", "templates", "_templates.md"), "utf8")).toContain("documents/_documents.md");
  });
...
`,
    ...
    await fs.writeFile(
      path.join(workflowRoot, "_external-review.md"),
      `---
open-forge:
...
`,
    ...
    const result = await runCli("find", "--route", ".agents/workflows/use-helper/_use-helper.md", "--follow-required", "--paths", payload);
