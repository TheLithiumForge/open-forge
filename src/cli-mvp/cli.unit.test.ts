...
import { cliTestInternals, createExtensionSelectionState, toggleExtensionSelection, type ExtensionDependencyInfo } from "./cli.ts";

describe("MVP extension catalogue selection state", () => {
...
`;
    const generatedChange = cliTestInternals.updateGeneratedIndexRegion(original, "- [Local route](local.md) - #Local", "fixture entrypoint");
    const authoredChange = original.replace("Authored contract v1.", "Authored contract v2.");

    expect(cliTestInternals.extensionOwnedFileSha256("fixture.md", generatedChange)).toBe(cliTestInternals.extensionOwnedFileSha256("fixture.md", original));
    expect(cliTestInternals.extensionOwnedFileSha256("fixture.md", authoredChange)).not.toBe(cliTestInternals.extensionOwnedFileSha256("fixture.md", original));
  });
...
`;
  ...
  test("requires a canonical source to contain only one complete managed block", () => {
    expect(() => cliTestInternals.patchMarkedBlock(null, "Missing markers.\n", "open-forge", "AGENTS.md")).toThrow(
      "Core source AGENTS.md must contain exactly one complete ordered open-forge marker pair",
    );

    expect(() =>
      cliTestInternals.patchMarkedBlock(
        null,
...
describe("Core Map topology", () => {
  test("accepts fresh, deliberately absent, and recognized current Map states", () => {
    expect(() => cliTestInternals.assertCoreMapTopologyState(false, false, false, false)).not.toThrow();
    expect(() => cliTestInternals.assertCoreMapTopologyState(false, false, true, false)).not.toThrow();
    expect(() => cliTestInternals.assertCoreMapTopologyState(false, true, true, true)).not.toThrow();
  });

  test("rejects legacy, dual-root, and unowned current Map states", () => {
    expect(() => cliTestInternals.assertCoreMapTopologyState(true, false, true, false)).toThrow("former Workspace primitive");
    expect(() => cliTestInternals.assertCoreMapTopologyState(true, true, true, false)).toThrow("both .agents/workspace/ and .agents/maps/");
    expect(() => cliTestInternals.assertCoreMapTopologyState(false, true, false, false)).toThrow("without recognizable Core anchors");
    expect(() => cliTestInternals.assertCoreMapTopologyState(false, true, true, false)).toThrow("without a recognized #Map entrypoint");
  });
});
...
`;
  ...
  test("accepts the minimal workflow contract and catches invalid optional dependencies", () => {
    ...
    const invalid = workflow.replace("- [Delivery capability](../skills/delivery/SKILL.md) - #Skill #Delivery", "- none");
    expect(cliTestInternals.validateWorkflowDocument(invalid)).toEqual(expect.arrayContaining([expect.stringContaining("must be omitted")]));
  });

  test("requires substantive direct directive Instructions and rejects legacy applicability", () => {
    expect(cliTestInternals.validateDirectiveDocument("# Safety\n\n## Instructions\n\n- Preserve user work.\n", false)).toEqual([]);
    expect(cliTestInternals.validateDirectiveDocument("# Safety\n\n## Applies To\n\n- src/**\n\n## Instructions\n\n- inherited\n", false)).toEqual(
      expect.arrayContaining([expect.stringContaining("scope belongs to routing"), expect.stringContaining("must be substantive")]),
    );
    expect(cliTestInternals.validateDirectiveDocument("# Safety\n\n## Instructions\n\n- Preserve user work.\n\n## Axioms\n\n- Legacy rule.\n", false)).toEqual(
      expect.arrayContaining([expect.stringContaining("Instructions instead of Axioms")]),
    );
  });
...
describe("portable extension paths", () => {
  test("accepts ordinary portable manifest and payload paths", () => {
    expect(() => cliTestInternals.assertPortableManifestPath(".agents/loader.md", "extension.json", "files[0]")).not.toThrow();
    expect(() => cliTestInternals.assertPortablePayloadPath(".agents/patterns/review/_review.md", "review-pack")).not.toThrow();
    expect(() => cliTestInternals.assertPortablePayloadPath(".agents/maps/repositories.md", "map-pack")).not.toThrow();
    expect(cliTestInternals.classifyExtensionPayloadPath(".agents/maps/repositories.md")).toBe("map");
  });

  test("rejects the retired Workspace primitive path without changing ordinary workspace scope", () => {
    for (const retired of [".agents/workspace/_workspace.md", ".AGENTS/WORKSPACE/routes.md"]) {
      expect(() => cliTestInternals.assertPortablePayloadPath(retired, "legacy-pack")).toThrow("retired Workspace primitive root");
    }
    expect(cliTestInternals.classifyExtensionPayloadPath("workspace-support/config.json")).toBe("other");
  });

  for (const invalid of ["CON/file.md", "folder/NUL.txt", "folder/COM1.log", "folder/trailing-dot.", "folder/trailing-space ", "folder/bad?.md", "folder/bad<name>.md"]) {
    test(`rejects Windows-unsafe path ${JSON.stringify(invalid)}`, () => {
      expect(() => cliTestInternals.assertPortableManifestPath(invalid, "extension.json", "files[0]")).toThrow("not portable across Windows");
      expect(() => cliTestInternals.assertPortablePayloadPath(invalid, "fixture-pack")).toThrow("not portable across Windows");
    });
  }

  test("rejects a literal backslash as a payload or manifest path separator", () => {
    const invalid = "folder/literal\\backslash.md";
    expect(() => cliTestInternals.assertPortableManifestPath(invalid, "extension.json", "files[0]")).toThrow("must be a portable relative path");
    expect(() => cliTestInternals.assertPortablePayloadPath(invalid, "fixture-pack")).toThrow("must be a portable relative path");
  });
});
