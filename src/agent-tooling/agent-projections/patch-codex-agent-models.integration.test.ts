...
test("merges authored and override policy before updating and checking a generated Codex agent", async () => {
  ...
  try {
    ...
    expect(patched).toEqual({ status: 0, stdout: "patched .codex/agents/review-agent.toml (review-agent)\nDone: 1 file(s) patched, 1 configured agent(s) matched.\n", stderr: "" });
    expect(await readFile(generatedAgent, "utf8")).toBe(ExpectedGeneratedAgent);

    const checked = runPatcher([...arguments_, "--check"], fixtureRoot);
    expect(checked).toEqual({ status: 0, stdout: "OK: 1 generated Codex agent model policies match.\n", stderr: "" });
    expect(await readFile(generatedAgent, "utf8")).toBe(ExpectedGeneratedAgent);
  } finally {
