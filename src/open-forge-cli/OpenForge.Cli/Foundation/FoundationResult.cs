...
internal sealed record FoundationFailure(
    FoundationFailureKind Kind,
    string Message);
...
internal sealed record FoundationReport(
    FoundationSnapshot Snapshot,
    string Json);

internal sealed record FoundationSnapshot(
    int SchemaVersion,
    int MarkdownHeadingCount,
    string YamlName,
    int FileByteCount,
    string FileSha256,
    bool ExclusiveLockObserved);
