...
---
//Hn: that description says everything, but is it the right tool to say everything like that? should we explore better options? Based on the rest of the content it is not bad at all. Also just an idea, should we group pur extensions based on what they do / are like architecture workflow in an workflows folder, capabilities in a skills folder and call them skills, mixed in some folder or root of extensions something like that. Also the vli needs big improvements in grouping them as well. We should have stable and unique names and ids (if we use rhem) as well refardless of where they sit. One more idea, since we have phases now for workflows, that means the agen should recommend/prompt/select a workflow based on what it understands the user wants, but it should also check beforehand if we should do an earlier phase beforehand to have more info, that automatically translates in the workflows or the extensions maybe to add some info somewhere about what they would need to be able to work correctly. That can be done and i do not know which is best, either indeting those comments in .ext.md files, i find this entire comments thing kind of annoying though, like i get the usefull ess and this is the simpliest way of not turning the framworks principles on their head (respectively and make sure we have memory/directives about this - we use top to bottom scopes, inheriting from the top, bubbling up some data only through our current description + tags mechanism, but we shall never be required to read a dynamicly loaded file beforehand just to know if we should use it or now, thst defeats the purpose - this can be a directive, ) - I'm digressing, probably the best way is to just extend a bit more our concepts to declare that a workflow has rhe following phases / lifecicles, and we should always check wehen we load a workflow if it has prerequisites declared, if it has indeed, we can recommend the user to do those and then proceed , qnd a workflow can have prerequisites in the forms of declared phases, or declared outputs from other phases, e.g. an architecture worflow should require a defined vision of the system but should work without as well. and another idea. this should practically by the end of it(and also vision i guess) should create via discussions with the user help create actual patterns/directives/guidance/memories/etc files such that we achieve way more than other frameworksfrom as little input and questions as possible. 
