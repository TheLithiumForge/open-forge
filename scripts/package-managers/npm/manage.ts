...
function stage(argumentsToRead: readonly string[]): void {
  const [artifactsRoot, runtimeValue, nativeArtifact, versionKind, versionValue, compiledLauncher] = argumentsToRead;
  if (
    ...
    (argumentsToRead.length !== 5 && argumentsToRead.length !== 6)
  ) {
  ...
  stagePackages({ repositoryRoot, artifactsRoot, runtime, nativeArtifact, version, ...(compiledLauncher === undefined ? {} : { compiledLauncher }) });
}
