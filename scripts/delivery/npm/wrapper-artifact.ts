...
export function recordWrapper(root: string, archive: string, version: string): void {
  ...
  writeFileSync(join(directory, "manifest.json"), `${JSON.stringify({ ...source, version, package: packed }, null, 2)}\n`);
}
